...
<!-- YANG INI EDITAN UNTUK TEST CETAK REPORT -->
		...
		<tr >
			<td colspan="9"  class="<?= $penanda ?>" ><span onclick="seeDetailIMO(this,'<?= $key ?>')" class="btn btn-xs btn-primary"> see detail >> </span>
				...
				<form method="post" target="_blank" id="form<?= $value['header']['WIP_ENTITY_NAME']; ?>" action="<?php echo base_url('InventoryManagement/CreateMoveOrderOPM/create'); ?>">
					...
					<tbody>
						...
						<tr class="baris<?=$no2?>">
							<td><?= $no2++; ?>
								...
								<input type="hidden" name="departement" value="<?= $value['header']['DEPT_CLASS'] ?>">
						...
						<?php 
							...
							$allDepartment[$no][] =  $value['header']['DEPT_CLASS'];
						?>
						<?php }
						else:?>
							<tr>
...
<!---- FORM KE 2 --------------------------------------------------->
			<form method="post" target="_blank" id="form2<?= $value['header']['WIP_ENTITY_NAME']; ?>" action="<?= base_url('InventoryManagement/CreateMoveOrderOPM/create') ?>" >
						<?php 
						$no2 = 1;
						if ($value['body']):
						foreach ($value['body'] as $kut => $vulue) { ?>
...
<div>
	<?php 
	if ($requirement) { ?>
	<form method="post" target="_blank" id="createall_picklist" action="<?php echo base_url('InventoryManagement/CreateMoveOrderOPM/createall'); ?>">
		...
		<input type="hidden" name="departement[]" value="<?= implode('<>', $allDepartment[$key]) ?>">
		<input type="hidden" name="piklis" value="1">
		<?php } ?>
	...
	<br><br>
	<form method="post" target="_blank" id="createall_plheader" action="<?php echo base_url('InventoryManagement/CreateMoveOrderOPM/createall'); ?>">
		<input type="hidden" name="selectedPicklistIMO" value="">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?= base_url('InventoryManagement/CreateMoveOrderOPM/create') ?>" id="form-modal<?= $value['header']['WIP_ENTITY_NAME']; ?>"
