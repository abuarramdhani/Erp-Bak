					...
					<div class="col-lg-12">
						<div class="box box-primary box-solid">
							...
							<div class="box-body">
								<div class="row">
									<div class="col-lg-12">
										<form class="form-horizontal" method="post" action="<?php echo site_url('CateringManagement/PengajuanLibur/Edit/'.$encrypted_link) ?>">
											<div class="form-group">
												<label class="control-label col-lg-4">Tanggal</label>
												<div class="col-lg-4">
													<input type="text" name="txtPengajuanLiburCateringCreate" id="txtPengajuanLiburCateringCreate" class="date form-control" placeholder="Tanggal"  disabled>
												</div>
											</div>
											<div class="form-group">
												<label class="control-label col-lg-4">Catering Libur</label>
												<div class="col-lg-4">
													<select class="select select2" name="txtLiburCatering" style="width: 100%" data-placeholder="Catering Libur" disabled>
														<option></option>
														<?php foreach ($Catering as $key) { ?>
															<option value="<?php echo $key['fs_kd_katering'] ?>" <?php $a = explode(" ", $isi['3']); if ($a['0'] == $key['fs_kd_katering']) {echo "selected"; }?>><?php echo $key['fs_nama_katering'] ?></option>
														<?php } ?>
													</select>
												</div>
											</div>
											<div class="form-group">
												...
												<div class="col-lg-4">
													<select class="select select2" name="txtPenggantiCatering" style="width: 100%" data-placeholder="Catering Pengganti" required>
														<option></option>
														<?php foreach ($Catering as $key) { ?>
															<option value="<?php echo $key['fs_kd_katering'] ?>"<?php $a = explode(" ", $isi['4']); if ($a['0'] == $key['fs_kd_katering']) {echo "selected"; }?>><?php echo $key['fs_nama_katering'] ?></option>
														<?php } ?>
													</select>
												</div>
											</div>
	...
	<script type="text/javascript">
		$(function(){
			...
			$('#txtPengajuanLiburCateringCreate').daterangepicker({
				...
				"maxDate": maxday,
				"startDate": '<?php echo $isi['0']." ".$isi['2'] ?>',
				"endDate": '<?php echo $isi['1']." ".$isi['2'] ?>'
	  		});
