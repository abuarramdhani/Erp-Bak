<section class="content">
	<div class="inner" >
	  	<div class="row">
	    	<div class="col-lg-12">
	      		<div class="row">
	        		<div class="col-lg-12">
	        			<div class="col-lg-11">
			            	<div class="text-right">
			            		<h1><b>Rekap Penerimaan Gaji Staff per Bank</b></h1>
			            	</div>
			          	</div>
			          	<div class="col-lg-1">
			            	<div class="text-right hidden-md hidden-sm hidden-xs">
			            		<a class="btn btn-default btn-lg" href="<?php echo site_url('PayrollManagement/');?>">
			            			<i class="icon-wrench icon-2x"></i>
			            			<span><br/></span>
			            		</a>
			            	</div>
			          	</div>
	        		</div>
	      		</div>
	      		<br/>
	      
			     <div class="row">
			        <div class="col-lg-12">
				        <div class="box box-primary box-solid">
				        	<div class="box-header with-border">
								<b>Rekap Penerimaan Gaji Staff per Bank</b>
				          	</div>
				          	<div class="box-body">
								<div class="row">
									<div class="row" style="margin: 10px 0px">
										<form method="post" action="<?php echo base_url('PayrollManagement/Report/RekapPenerimaanGajiStaff/search')?>" enctype="multipart/form-data">						
											<div class="col-lg-2">
												<input name="txtPeriodeHitung" placeholder="Periode" id="txtPeriodeHitung" type="text" class="form-control">
											</div>
											<div class="col-xs-2">
												<select name="txtBank" class="select2-txtBank">
													<option value="BCA" selected>Bank BCA</option>
													<option value="WINDU">Bank WINDU</option>
												</select>
											</div>
											<div class="col-lg-2">
												<button class="btn btn-warning btn-block">Search</button>
											</div>
										</form>
										<div class="col-lg-3"></div>
									</div>
								</div>
								<?php if ($period_shown) 
									{
										echo '<div class="row" style="margin: 10px 0px">
											<table>
												<tr>
													<td width="50px">Periode</td>
													<td> :&nbsp</td>
													<td>'.$year.' - '.$month.'</td>
												</tr>
												<tr>
													<td width="50px">Bank</td>
													<td> :&nbsp</td>
													<td>'.$bank.'</td>
												</tr>
												<tr>
													<td width="50px">Total</td>
													<td> :&nbsp</td>
													<td>Rp. '.number_format($total['total_thp'],0,",",".").'</td>
												</tr>
												<tr>
													<td width="50px">PDF</td>
													<td> :&nbsp</td>
													<td><a class="btn btn-xs btn-primary" href="'.site_url('PayrollManagement/Report/RekapPenerimaanGajiStaff/GeneratePDF?bank='.$bank.'&year='.$year.'&month='.$month).'">'.
														'<span class="fa fa-file-pdf-o"></span></a></td>
												</tr>
											</table>
										</div>';
									}
								?>
					            <div class="table-responsive">
					              	<table class="table table-striped table-bordered table-hover text-left" style="font-size:12px;">
						                <thead class="bg-primary">
							                <tr>
							                    <th style="text-align:center"><div style="width:40px"></div>NO</th>
							                    <th style="text-align:center"><div style="width:100px"></div>No Induk</th>
												<th style="text-align:center"><div style="width:100px"></div>Nama</th>
												<th style="text-align:center"><div style="width:100px"></div>No Rekening</th>
												<th style="text-align:center"><div style="width:100px"></div>Nama Pemilik Rekening</th>
												<th style="text-align:center"><div style="width:100px"></div>THP</th>
							                </tr>
						                </thead>
						                <tbody>
											<?php $no = 1; 
											if(!empty($data_gaji)){
												foreach($data_gaji as $row) { ?>
												<tr>
													<td align='center'><?php echo $no++;?></td>
													<td align='center'><?php echo $row['no_induk'] ?></td>
													<td align='center'><?php echo $row['nama'] ?></td>
													<td align='center'><?php echo $row['no_rekening'] ?></td>
													<td align='center'><?php echo $row['nama_pemilik_rekening'] ?></td>
													<td align='center'><?php echo 'Rp. '.number_format($row['thp'],0,",",".") ?></td>
												</tr>
											<?php 
												}
											}
											?>
						                </tbody>                                      
					              	</table>
					            </div>
					            <?php if ($period_shown) 
									{
										echo '<div class="row" style="margin: 10px 0px">
											<table>
												<tr>
													<td width="180px">Total Penerimaan Gaji</td>
													<td> :&nbsp</td>
													<td>Rp. '.number_format($total['total_thp'],0,",",".").'</td>
												</tr>
												<tr>
													<td width="180px">Total Potongan Transfer</td>
													<td> :&nbsp</td>
													<td>Rp. '.number_format($total['total_pot_transfer'],0,",",".").'</td>
												</tr>
												<tr>
													<td width="180px">Total Setelah Potongan</td>
													<td> :&nbsp</td>
													<td>Rp. '.number_format($total['total_nett'],0,",",".").'</td>
												</tr>
											</table>
										</div>';
									}
								?>
					        </div>
					    </div>
				    </div>
			    </div>    
			</div>    
		</div>
	</div>
</section>