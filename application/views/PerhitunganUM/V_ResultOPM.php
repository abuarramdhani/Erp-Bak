                    ...
                    <div class="col-lg-12">
						<div class="col-lg-11">
							<div class="text-right">
								<h1>
									<b>
										Perhitungan Utilitas Mesin OPM
									</b>
								</h1>
							</div>
						</div>
                    ...
                    <div class="col-lg-12">
                        <div class="box box-primary">
                            <div class="box-header with-border text-center">
								<h3><b>DATA PERHITUNGAN UTILITAS MESIN OPM</b></h3>
								<h4><b><?= $classrout?></b></h4>
							</div>
                            <div class="box-body">
                                <div class="col-md-12">
                                    <form action="<?php echo base_url('PerhitunganUM/HitungOPM/exportDataPUM'); ?>" method="post">
                                        <table id="tblduaopm" class="table table-striped table-bordered table-hover" style="font-size: 13px;">
                                            <thead class="bg-info">
                                                <tr>
											...
											<?php $no=1;foreach($result as $row){ ?> 
												...
												<tr style="text-align: center; vertical-align: middle;">                        
													<td><?= $no; ?>
														<input type="hidden" name="classrout[]" id="classrout" value="<?= $classrout ?>">
													...
													<td><input type="hidden" name="mesin[]" id="mesin" value="<?= $v['mesin']?>"><?= $v['mesin']?></td>               
													...
													<td><input type="hidden" name="item_desc[]" id="item_desc" value="<?= $v['item_desc']?>"><?= $v['item_desc']?></td>          
                                                    ...
                                                    <?php } else if ($v['mesin'] == null) { ?>
                                                    <td><input type="hidden" name="total_jam[]" id="total_jam" value=""></td> 
                                                    <?php } ?>
                                                    <?php if ($v['mesin'] != null) { ?>
                                                    <td><input type="hidden" name="utilitas[]" id="utilitas" value="<?= $row['utilitas']?>"><?= $row['utilitas']?></td> 
                                                    <?php } else if ($v['mesin'] == null) { ?>
                                                    <td><input type="hidden" name="utilitas[]" id="utilitas" value=""></td> 
                                                    <?php } ?>
													<!-- <td><input type="hidden" name="total_jam[]" id="total_jam" value="<?= $row['totaljam']?>"><?= $row['totaljam']?></td>
													<td><input type="hidden" name="utilitas[]" id="utilitas" value="<?= $row['utilitas']?>"><?= $row['utilitas']?>%</td> -->
												</tr>                    
