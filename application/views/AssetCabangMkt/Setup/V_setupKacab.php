			...
			<div class="col-lg-12">
				<div class="row">
					<div class="col-lg-12">
						<div class="col-lg-12">
							<div class="text-left">
								<span><h2><b>Super User</b> - Atur Kepala Cabang</h2></span>
							</div>
						</div>
					</div>
				</div>
										...
										<table  id="filter" class=" tblResponsive" style="margin-bottom: 20px">
											<tr>
												...
												<td style="width:35%; padding: ">
													<input type="text" style="width:300px" placeholder="Masukkan Nomor Induk" class="form-control capital" id="txtNomorIndukAC">
												</td>
												<td style="width:45%; padding-left:10px ">
													<button type="button" class="btn btn-primary btn-sm"  style="width: 100px;margin-left: 10px" id="btnSubmitSetup" onclick="cariNomorIndukKacab(this)" ><i class="fa fa-check"></i> Cari </button> 
												</td>
											</tr>
											<tr>
											...
											<tr>
												...
												<td style="padding-top: 10px;">
													<select required id="slcAsalCabangSU" name="slcAsalCabangSU" class="form-control select2 select2-hidden-accessible capital" style="width:300px;" required="required">
											...
											<tr>
												<td style="width: 20%;padding-top: 10px;padding-left: 20px">
													<span><b>Status Kacab</b></span>
												</td>
												<td style="padding-top: 10px;">
													<select id="slcStatusKacab" name="slcStatusKacab" class="form-control select2 select2-hidden-accessible capital" style="width:300px;" required="required">
						                                    <option value="" > Pilih Status Kacab </option>
						                                    <option value="Y"> Aktif </option>
						                                    <option value="N"> Non-aktif </option>
                  									</select>
												</td>
												<td style="width:45%; padding-left:10px;padding-top: 10px;">
													<button type="button" class="btn btn-success btn-sm"  style="width: 100px;margin-left: 10px" id="btnSubmitSetup" onclick="saveKacab(this)" ><i class="fa fa-check"></i> Save </button> 
												</td>
											</tr>
										</table>
								...
								<table class="table table-responsive table-striped table-bordered table-hover text-center" id="tblACLDAO" style="font-size:14px;width:100%">
										<thead>
											<tr style="background-color: #d2d6de; color:black;" class="bg-primary">
												...
												<th width="15%"><center>Nama Kacab</center></th>
										...
										<tbody id="tambahJA">
											<?php $i=1; foreach ($ss as $k) {?> 
												<tr class="<?= $k['id_kacab']?>" >
													<td class="no"><?= $i;?></td>
													<td style="display: none"><input type="hidden" id="hdnIDAC" value="<?= $k['id_kacab']?>"></td>
													<td><?= $k['no_induk'];?></td>
													<td><?= $k['nama_kacab'];?></td>
													<td><?= $k['kode_cabang'];?></td>
													<td><?= $k['section_name'];?></td>
													<td><?php if ($k['status'] == 'Y') { ?>
													<span><label class="label label-success">Active</label></span>
													<?php }else if ($k['status'] == 'N'){ ?>
													<span><label class="label label-danger">Inactive</label></span>
													<?php } ?>
													</td>	
													<td>
														<button type="button" onclick="deleteKacab(<?= $k['id_kacab']?>)" class="btn btn-sm btn-danger" style="width:100px;margin-bottom: 5px"><i class="fa fa-trash"></i> Delete</button>
														<button type="button" data-target="#mdlSuperUser" data-toggle="modal" class="btn btn-sm btn-primary" onclick="openEditKacab(<?php echo $k['id_kacab']?>)" style="width:100px"><i class="fa fa-pencil"></i> Edit</button>
													</td>
												</tr>
											<?php $i++;} ?> 
