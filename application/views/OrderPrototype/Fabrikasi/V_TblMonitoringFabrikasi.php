<table class="table table-bordered" id="mon_order2">
									<thead class="bg-yellow">
										<tr>
											<th class="text-center bg-yellow">Status</th>
											<th class="text-center bg-yellow">No Order</th>
											<th class="text-center bg-yellow">Kode Komponen</th>
											<th class="text-center bg-yellow">Nama Komponen</th>
									...
									<tbody>
										...
										<tr class="<?=$background?>">
											...
											<?php } else { ?>
											<td class="text-center"><button class="btn btn-success btn-xs" onclick="terimamaterial(<?=$ord?>)">Terima</button></td>
											<?php } ?>
											...
											<?php } else { ?>
											<td class="text-center"><button onclick="inputtgljob(<?=$ord?>)" class="btn btn-warning btn-xs">Input</button></td>
											<?php } ?>

											<td class="text-center"><button onclick="lihatprogresss(<?=$ord?>)" class="btn btn-primary btn-xs">Detail</button></td>

											<?php if ($mon['last_updated_date'] != null) { ?>
												<td class="text-center"><input type="hidden" value="<?=date('d-M-Y',strtotime($mon['last_updated_date']))?>" name=""><?=date('d-M-Y',strtotime($mon['last_updated_date']))?> <?=date('H:i:s',strtotime($mon['last_updated_time']))?></td>
											<?php } else { ?>
											<td class="text-center">-</td>
											<?php } ?>
											...
											<?php } else { ?>
											<td class="text-center"><button onclick="pic_fabrikasi(<?=$ord?>)" class="btn bg-teal btn-xs">Input</button></td>
											<?php } ?>
											...
											<?php } else { ?>
											<td class="text-center"><button onclick="inputqtyfinish(<?=$ord?>)" class="btn bg-orange btn-xs">Input</button></td>
											<?php } ?>

											
											<?php if ($mon['kirim'] == null && $mon['qty_finish'] != null ) { ?>
											<td class="text-center"><button onclick="kirim(<?=$ord?>)" class="btn btn-danger btn-xs">Kirim</button></i></td>
											<?php } else if ($mon['kirim'] == null && $mon['qty_finish'] == null ) { ?>
											<td class="text-center"><i style="color: red" class="fa fa- fa-remove fa-2x"></i></td></td>
											<?php } else { ?>
											...
											<?php if ($mon['terima'] == null) { ?>
											<td class="text-center"><i style="color: red" class="fa fa- fa-remove fa-2x"></td>
											<?php } else if ($mon['terima'] != null) { ?>
											<td class="text-center"><i style="color: green" class="fa fa- fa-check fa-2x"></i><br><?=date('d-M-Y H:i:s',strtotime($mon['tgl_act_terima']))?></td>
											<?php }?>
