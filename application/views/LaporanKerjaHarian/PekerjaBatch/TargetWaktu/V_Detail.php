                        ...
                        <div class="box box-primary box-solid">
							...
							<div style="padding: 12px 18px 4px 18px; background-color: #3C8DBC; color: white;" class="box-footer" <?= (empty($recordPekerjaan) && empty($rerataGolonganKondite) && empty($warningSP)) ? 'hidden' : ''; ?>>
								...
								<ul style="margin-top: 6px;">
									...
									<?= (empty($warningSP)) ? '' : '<li style="color: red; font-weight: bold;" id="txt-warning-sp">Mendapat SP Bawah Prestasi karena tanggal '.$warningSP.' tidak mencapai target.</li>'; ?>
								<ul>
							</div>
                        </div>
                    </div>
...
<script type="text/javascript">
    var employee_name = '<?= $EmployeeName ?>'; var employee_code = '<?= $filterPekerja ?>'; var stringCache = '';
	$(function() {
    ...
    function onFocusOut(input) { if(stringCache != input.val()) updateData($('#lkh-id-row-' + input.data('row')).text(), input.data('column'), input.val(), input); }
