...
<table class="datatable table table-bordered table-hover table-striped tbpickl text-center" style="width: 100%;">
    ...
    <tbody>
    <?php $no = 1; $i= 1;foreach ($data as $val) { ?>
        <tr>
            <td><input type="hidden" id="jmli" class="jmli" value="<?= $i?>"><?= $no?></td>
            ...
            <!-- <td><input type="button" class="btn btn-warning" id="cek<?= $no?>" value="cek" onclick="cekfabrikasi(<?= $no?>)"></td> -->
            <td><button type="button" class="btn btn-success" id="appfab<?= $no?>" onclick="approveFabrikasi(<?= $no?>)">Approve</button></td>
        </tr>
    <?php $no++; }?>
    </tbody>
</table>
