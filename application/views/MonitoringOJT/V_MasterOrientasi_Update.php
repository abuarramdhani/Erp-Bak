				...
				<div class="col-lg-12">
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="box box-primary box-solid <?php if($masterOrientasi['memo']=='0'){echo 'hidden';};?>" id="MonitoringOJT-PengaturanUndanganOrientasiBaru">
								<div class="box-header with-border">
										...
										<div class="row">
											<div class="form-group">
												...
												<select class="select2 col-lg-6" name="cmbFormatCetak" id="MonitoringOJT-cmbFormatCetak">
													<option value="<?php echo $masterOrientasi['id_memo'];?>" selected><?php echo $masterOrientasi['memo_judul'];?></option>
												</select>
											</div>
											<table class="table" style="width: 100%" id="MonitoringOJT-pengaturanUndangan">
												...
												<?php
													$indeks_undangan 	=	0;
													foreach ($editUndanganOrientasi as $undangan) 
													{
														$id_undangan 			=	$undangan['id_berkas'];
														$id_undangan_encode		=	$this->encrypt->encode($id_undangan);
														$id_undangan_encode 	=	str_replace(array('+', '/', '='), array('-', '_', '~'), $id_undangan_encode);
												?>
												<tr>
													<td>
														<a class="btn btn-sm btn-danger" onclick="MonitoringOJT_hapusUndangan(this)" ><i class="fa fa-times"></i></a>
														<input class="hidden" type="text" name="idUndangan[<?php echo $indeks_undangan;?>]" value="<?php echo $id_undangan_encode;?>" />
													</td>
													<td>
														<input type="number" name="numUndanganIntervalHari[<?php echo $indeks_undangan;?>]" class="form-control" min="0" max="99" step="1" placeholder="Hari" value="<?php echo $undangan['hari'];?>"/>
													</td>
													<td>
														<input type="number" name="numUndanganIntervalMinggu[<?php echo $indeks_undangan;?>]" class="form-control" min="0" max="10" step="1" placeholder="Minggu" value="<?php echo $undangan['minggu'];?>" />
													</td>
													<td>
														<input type="number" name="numUndanganIntervalBulan[<?php echo $indeks_undangan;?>]" class="form-control" min="0" max="10" step="1" placeholder="Bulan" value="<?php echo $undangan['bulan'];?>" />
													</td>
													<td>
														<select class="select2 form-control MonitoringOJT-cmbUndanganPelaksanaan" style="width: 100%" name="cmbUndanganPelaksanaan[<?php echo $indeks_undangan;?>]" placeholder="Pelaksanaan">
															<option></option>
															<option value="0" <?php if($undangan['urutan']=='0'){echo 'selected';};?>>Sebelum</option>
															<option value="1" <?php if($undangan['urutan']=='1'){echo 'selected';};?>>Sesudah</option>
														</select>
													</td>
													<td>
														<select class="select2 form-control MonitoringOJT-cmbUndanganTujuan" style="width: 100%" name="cmbUndanganTujuan[<?php echo $indeks_undangan;?>]" placeholder="Tujuan">
															<option></option>
															<option value="1" <?php if($undangan['penerima']=='1'){echo 'selected';};?>>People Development</option>
															<option value="2" <?php if($undangan['penerima']=='2'){echo 'selected';};?>>Pekerja</option>
															<option value="3" <?php if($undangan['penerima']=='3'){echo 'selected';};?>>Atasan Pekerja</option>
														</select>
													</td>
												</tr>
												<?php
