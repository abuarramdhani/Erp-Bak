...
<table class="datatable table table-bordered table-hover table-striped text-center text-nowrap" id="tb_cuttingresharp" style="width: 100%;">
    <thead class="bg-teal">
        <tr>
    ...
    <tbody>
        <?php $no = 1; foreach ($data as $key => $val) {?>
            <tr>
                <td><?= $no?></td>
                <td><input type="hidden" name="item_resharp<?= $no?>" value="<?= $val['ITEM']?>"><?= $val['ITEM']?></td>
                <td><input type="hidden" name="desc_resharp<?= $no?>" value="<?= $val['DESCRIPTION']?>"><?= $val['DESCRIPTION']?></td>
                ...
                <td><?= $val['TOTAL_IN']?></td>
                <td><?= $val['TOTAL_OUT']?></td>
                <td><select class="form-control select2 databulan" name="bulan_resharp<?= $no?>" style="width:120px">
                        <option value="<?= date('M-Y')?>"><?= date('F')?></option>
                    </select>
                    <button style="margin-left:5px" type="submit" class="btn btn-info" formaction="<?php echo base_url("MonitoringCuttingTool/MonitoringTransaksi/DetailTransaksiResharp/".$no."")?>"><i class="fa fa-eye"></i> Detail</button>
                </td>
            </tr>
        <?php $no++; }?>
    </tbody>
