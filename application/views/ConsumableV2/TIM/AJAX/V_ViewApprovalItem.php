<table class="table table-bordered tbl_cst_apprkeb" style="width:100%;text-align:center">
  <thead style="background:#05a6f0;color:white">
    <tr>
  ...
  <tbody>
    <?php foreach ($approvalkebutuhan as $key => $value): ?>
      <tr>
        ...
        <td>
          <button type="button" class="btn" name="button" data-toggle="modal" data-target="#detail-approval-cst-item" title="detail" onclick="detailapprovalcstitem('<?php echo $value['KODESIE'] ?>', '<?php echo $value['SEKSI'] ?>')"> Detail Item</button>
        </td>
      </tr>
    <?php endforeach; ?>
  </tbody>
