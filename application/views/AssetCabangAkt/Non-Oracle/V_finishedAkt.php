									...
									<table class="table table-striped table-bordered table-hover text-center" id="tblACLDAO" style="font-size:14px;">
										<thead>
											<tr style="background-color: #d2d6de; color:black;" class="bg-primary">
												...
												<th width="10%"><center>Seksi Pemakai</center></th>
										...
										<tbody>
											<?php $i=1; foreach ($finished as $k) {?>
												<tr>
													...
													<td><?= $k['nama_ka'];?></td>
													<td><?= $k['nama_ja'];?></td>
													<td><?= $k['seksi_pemakai'];?></td>
													<?php if ($k['status'] == '5') { ?>
													<td>
													<span><label class="label label-success"><i class="fa fa-check"></i>&nbsp;Input to Oracle by Akuntansi</label></span>
													</td>
													<?php } ?>
