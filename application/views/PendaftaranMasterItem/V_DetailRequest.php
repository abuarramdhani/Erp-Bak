<div class="panel-body" style="margin-top:-20px">
    <h2 style="color : #3F5359;text-align:center;font-weight:bold"><?= $header['no_dokumen']?></h2>
        ...
        <table class="table table-bordered table-hover table-striped text-center" id="tbldetail" style="width: 100%;">
            <thead class="text-nowrap" style="background-color:#49D3F5">
                <tr>
                    ...
                    <?php // tambahan kolom kalau menu finished resp pendaftaran master item dan performed resp. master item piea
                        if ($ket == 'finished' || $ket == 'performpiea') { ?>
                        <th rowspan="2" style="vertical-align:middle;">Finished</th>
                    <?php }?>
                </tr>
                <tr>
                    <th>ODM</th>
            ...
            <?php $no = 1; foreach ($data as $key => $val) {?>
                <tr>
                    ...
                    <td><input type="hidden" id="id_item<?= $no?>" name="id_item[]" value="<?= $val['ID_ITEM']?>"><?= $val['ITEM_FIX']?></td>
                    <td><?= $val['DESC_FIX']?></td>
                    <td><?= $val['KODE_UOM']?></td>
                    ...
                    <?php if ($ket == 'finished' || $ket == 'performpiea') { ?>
                        <td><?= $val['FINISH'] == 'ACCEPT' ? '<span style="color:green;font-weight:bold">OK</span>' : 
                                ($val['FINISH'] == 'REJECT' ? '<span style="color:red;font-weight:bold">NOT OK</span>' : '') ?></td>
                    <?php }?>
                </tr>
            <?php $no++; }?>
            </tbody>
