						...
						<div class="box box-primary box-solid">
							<div class="box-header with-border">
								...
								if(in_array($no_ind, $admin)){
									...
									<a href="<?php echo site_url('Warehouse/Transaksi/CreatePeminjamanConsumable') ?>" style="float:right;margin-right:1%;margin-top:-0.5%;" alt="Add New" title="Add New" >
								...
								<table class="table table-striped table-bordered table-hover text-left table-item-usable" id="table-item-usable" style="font-size:12px;">
									<thead>
										<tr class="bg-primary">
											...
											<th width="5%"><center>Noind</center></th>
									...
									<tbody>
										...
										<tr>
											<td align="center">
												...
												if(in_array($no_ind, $admin)){
													?>
													<a class="btn btn-xs bg-green" href="<?php echo base_url('Warehouse/Transaksi/UpdateItemConsumable/')."/".$encrypted_string."/".$ListOutGroupTransaction_item['creation_date'] ?>"><span title="Update Peminjaman <?php echo $ListOutGroupTransaction_item['noind'] ?>" class="fa fa-edit"></span></a>
													<a class="btn btn-xs bg-maroon" href="<?php echo base_url('Warehouse/Transaksi/RemoveItemConsumable/')."/".$encrypted_string."/".$ListOutGroupTransaction_item['creation_date'] ?>" onclick="return confirm('are you sure to delete this tool ?')"><span title="Delete Peminjaman <?php echo $ListOutGroupTransaction_item['noind'] ?>" class="fa fa-remove"></span></a>
													<?php 

												} 
												?>
												<a class="btn btn-xs bg-blue" href="<?php echo base_url('Warehouse/Transaksi/ListItemConsumable/')."/".$encrypted_string."/".$ListOutGroupTransaction_item['creation_date'] ?>"><span title="View List Peminjaman <?php echo $ListOutGroupTransaction_item['noind'] ?>" class="fa fa-search"></span></a>
											</td>
											...
											<td align="center"><?php echo $ListOutGroupTransaction_item['item_id'] ?></td>
											<td align="center"><?php echo $ListOutGroupTransaction_item['item_qty'] ?></td>
										</tr>
									<?php endforeach ?>
								</tbody>
							</table>
