  ...
  <table class="table table-bordered table-striped table-hover text-left datatable-handling" style="font-size:13px">
    <thead>
      <tr class="bg-primary">
        ...
        <th style="text-align:center;width:15%;">Point Penyimpangan</th>
    ...
    <tbody>
      <?php $no = 1; foreach ($get_audit_handling as $key => $value):
        ...
        <?php
          if (!empty($get_audit_handling[$key+1]) && $value['id'] == $get_audit_handling[$key+1]['id']) {
            $alasan_masih_open = $get_audit_handling[$key+1]['alasan_masih_open'];
          }else {
            $alasan_masih_open = '';
          }
         ?>
         <?php if ($key >= 1):
           $keyq = $key-1;?>
           <?php if ($value['id'] == $get_audit_handling[$keyq]['id']) {
             continue;
           } ?>
      <tr style="<?php echo $value['status'] != 'CLOSE' ? '' : 'background-color:rgb(48 222 134);color:black'?>">
        ...
        <td style="text-align:center;min-width:120px;vertical-align:middle">
          <a class="btn btn-info"
            href="<?php echo base_url('MenjawabTemuanAudite/Handling/edit/'.$encrypted_string2.' '.$encrypted_string.'') ?>" title="Answer Audite" style="margin-right:4px;<?php echo $value['status'] == 'CLOSE' ? 'display:none' : '' ?>">
            <i class="fa fa-edit"></i>
          </a>
          <a class="btn btn-danger"
            href="<?php echo base_url('MenjawabTemuanAudite/Handling/pdfHandling/'.$encrypted_string2.' '.$encrypted_string.'') ?>" target="_blank" title="Export Data to PDF" style="<?php //echo $value['id'] == $get_audit_handling[$keyq]['id'] ? 'display:none' : '' ?>">
      ...
      <?php else: ?>
        <tr style="<?php echo $value['status'] != 'CLOSE' ? '' : 'background-color:rgb(48 222 134);color:black'?>">
          ...
          <td style="text-align:center;min-width:120px;vertical-align:middle">
            ...
            <a class="btn btn-danger"
              href="<?php echo base_url('MenjawabTemuanAudite/Handling/pdfHandling/'.$encrypted_string2.' '.$encrypted_string.'') ?>" target="_blank" title="Export Data to PDF">
              <i class="fa fa-file-pdf-o"></i>
            </a>
          </td>
        </tr>
...
<script type="text/javascript">
  ...
  $('#showbefore').on('show.bs.modal', function(e) {
    ...
    $.ajax({
      url: baseurl + 'MenjawabTemuanAudite/Handling/getGambarBefore',
      type: 'POST',
      data: {
        id: getGambar,
      },
  ...
  $('#showafter').on('show.bs.modal', function(e) {
    var getGambar = $(e.relatedTarget).data('id');
    $.ajax({
      url: baseurl + 'MenjawabTemuanAudite/Handling/getGambarAfter',
      type: 'POST',
      data: {
        id: getGambar,
      },
