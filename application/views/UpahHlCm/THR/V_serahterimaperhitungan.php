...
<div style="margin: 10px;">
	<div style="width: 100%; height: 20px; border: 1px solid black; font-size: 14px;text-align: center;padding-top: 5px;">
		<p>SLIP TUNJANGAN HARI RAYA PEKERJA HARIAN <?php if($i=='0'){echo "PUSAT";}else{echo "TUKSONO";}?><br> TANGGAL : 
		<?php 
		$month1=date('m',strtotime($tanggal));
		...
		<table style="margin-top: 50px;font-size: 13px; text-align: center; width: 100%; text-transform: lowercase;">
			<tr>
				<td style="width: 30%;text-align: center;">
					...
					foreach ($pj as $dt) {
						if ($i == '0') {
							if ($dt['id_status'] == '3' and $dt['lokasi_kerja'] == '01') {
								$nama = strtolower($dt['nama']);
								echo ucwords($nama);
							}
						}else {
							if ($dt['id_status'] == '3' and $dt['lokasi_kerja'] == '02') {
								$nama = strtolower($dt['nama']);
								echo ucwords($nama);
							}
						} 
				...
				<td style="width: 35%;text-align: center;">
					...
					foreach ($pj as $dt) {
						if ($i == '0') {
							if ($dt['id_status'] == '2' and $dt['lokasi_kerja'] == '01') {
								$nama = strtolower($dt['nama']);
								echo ucwords($nama);
							}
						}else {
							if ($dt['id_status'] == '2' and $dt['lokasi_kerja'] == '02') {
								$nama = strtolower($dt['nama']);
								echo ucwords($nama);
							}
						} 
				...
				<td>
					...
					foreach ($pj as $dt) {
						if ($i == '0') {
							if ($dt['id_status'] == '1' and $dt['lokasi_kerja'] == '01') {
								$nama = strtolower($dt['nama']);
								echo ucwords($nama);
							}
						}else {
							if ($dt['id_status'] == '1' and $dt['lokasi_kerja'] == '02') {
								$nama = strtolower($dt['nama']);
								echo ucwords($nama);
							}
						} 
