...
<section class="content" id="detailmiscellaneous">
    <div class="inner">
        <div class="row">
            <div class="col-lg-12">
                <br />
                <?php if ($ket == 'approvemanualCosting') {?>
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="box-header text-center" style="font-size:20px">
                            ...
                            <div class="box-body">
                                <div class="panel-body text-center">
                                    <img style="max-width: 700px;max-height: 700px" src="<?= base_url("assets/upload/Miscellaneous/Approve_Manual/".$id_header."-".$requester.".png")?>">
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <table class="datatable table table-bordered table-hover table-striped text-center" id="<?= $datatbl?>">
                                        ...
                                        <?php $no = 1; foreach ($data as $key => $val) {
                                            if ($val['status'] == '') { // item accept
                                                ...
                                                $iditem = '<input type="hidden" name="id_item[]" value="'.$val['id_item'].'">
                                                            <input type="hidden" id="revqty'.$no.'" name="revqty[]">';
                                                $qty    = '';
                                            } else { // item reject
                                        ...
                                        ?>
                                            <tr>
                                                <td><?= $no?>
                                                    <?= $iditem?>
                                                    <input type="hidden" id="kode_item<?= $no?>" value="<?= $val['kode_item']?>">
                                                    <input type="hidden" id="onhand<?= $no?>" value="<?= $val['onhand']?>">
                                                    <input type="hidden" id="qty<?= $no?>" value="<?= $val['qty']?>">
                                                </td>
                                                <td><?= $val['issue_receipt']?></td>
                                                <td><?= $val['kode_item']?></td>
                                                <td><?= $val['deskripsi_item']?></td>
                                                <td><?= $val['qty']?> 
                                                    <button type="button" class="btn btn-xs bg-orange" onclick="revisiqty(<?= $no?>)" style="<?= $qty?>"><i class="fa fa-edit"></i></button>
                                                </td>
                                                <td><?= $val['uom']?></td>
                                                <td><?= $val['inventory']?></td>
                                                <td><?= $val['locator']?></td>
