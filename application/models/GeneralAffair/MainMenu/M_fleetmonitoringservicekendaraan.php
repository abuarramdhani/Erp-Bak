...
class M_fleetmonitoringservicekendaraan extends CI_Model
{
    ...
    public function ambilSudah()
    {
        return $this->db->get('ga.ga_fleet_monitoring_service')->result_array();
    }
    public function insertMonitoring($data)
    {
        $this->db->insert('ga.ga_fleet_monitoring_service',$data);
        return;
    }
    public function ambilJenisService()
    {
        $data = "select * from ga.ga_fleet_jenis_service order by jenis_service_id asc";
        return $this->db->query($data)->result_array();
    }
    public function getServiceKendaraan()
    {
        return $this->db->get('ga.ga_fleet_service_kendaraan')->result_array();
    }
    public function getFleetserviceKendaraan()
    {
        $ambilKendaraan     = " select  a.nomor_polisi,
                                        a.merk_kendaraan_id as id_merk,
                                        a.kendaraan_id as k_id,
                                        b.merk_kendaraan,
                                        min(c.kilometer) as kilometer,
                                        min(c.bulan) as bulan,
                                        (select max(z.kilometer) from ga.ga_fleet_histori_pemakaian z where z.nomor_polisi=a.nomor_polisi) as km_saat_ini
                                from    ga.ga_fleet_kendaraan as a
                                        join ga.ga_fleet_merk_kendaraan b 
                                         on a.merk_kendaraan_id=b.merk_kendaraan_id
                                        join ga.ga_fleet_service_kendaraan c 
                                         on a.merk_kendaraan_id=c.merk_kendaraan_id
                                GROUP BY a.merk_kendaraan_id,a.kendaraan_id,b.merk_kendaraan
                                        ";
    ...
    public function getMonitoringKendaraanDetail($periode1,$periode2,$query_lokasi)
    {
        $query = $this->db->query("select   mtckdrn.maintenance_kendaraan_id as kode_maintenance,
                                            kdrn.nomor_polisi as nomor_polisi,
                                            to_char(mtckdrn.tanggal_maintenance, 'DD-MM-YYYY') as tanggal,
                                ...
                                where       mtckdrn.tanggal_maintenance between '$periode1' and '$periode2'
                                            and     mtckdrn.end_date='9999-12-12 00:00:00' $query_lokasi
                                order by    tanggal_asli desc");
        return $query->result_array();
    }
