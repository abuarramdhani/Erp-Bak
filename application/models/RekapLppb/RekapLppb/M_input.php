...
class M_input extends CI_Model
{
    ...
    public function getDataRekap($lppb, $io){
        $sql = "SELECT rec.*, 
...
--                AND rec.item_id = ww.ITEM_ID(+)
    ...
    public function getDataRekap2($bulan, $io){
        $sql = "SELECT rec.*, 
        nvl(to_char(ww.kirim_qc,'DD/MM/YYYY HH24:MI:SS'),null) kirim_qc, 
        nvl(to_char(ww.terima_qc,'DD/MM/YYYY HH24:MI:SS'),null) terima_qc,
        nvl(to_char(ww.kembali_qc,'DD/MM/YYYY HH24:MI:SS'),null) kembali_qc, 
...
--                                        pha.segment1 po, 
                                ...
                                WHERE rsh.ship_to_org_id = '$io'
                                    ...
                                    -- parameter---------------------------------
                --                    and pha.SEGMENT1 = '19018662'
                                   and to_char(rsh.CREATION_DATE ,'MON-YYYY') = nvl('$bulan',to_char(rsh.CREATION_DATE , 'MON-YYYY'))
                --                    and msib.INVENTORY_ITEM_ID = ''
                --                    and rsh.RECEIPT_NUM between nvl(:P_LPPB_FROM,rsh.RECEIPT_NUM) and nvl(:P_LPPB_TO,rsh.RECEIPT_NUM)
                                    ----------------------------------------------
                                    AND rsh.shipment_header_id = rt.shipment_header_id
...
--                AND rec.item_id = ww.ITEM_ID(+)
    ...
    public function cekdata($recnum, $io){
		$sql= "SELECT * from KHS_WAKTU_LPPB where receipt_num='$recnum' and io = '$io'";
        $query = $this->oracle->query($sql);                             
        return $query->result_array();
    }
    
    public function Updatedata($recnum,$queryupdate, $io){
    ...
    public function Insertdata($recnum,$queryinsert,$ket,$io){
		$sql= "INSERT INTO KHS_WAKTU_LPPB (receipt_num, io , $ket)
        values ('$recnum','$io',$queryinsert)";
        $query = $this->oracle->query($sql);                             
        echo $sql;
    }
}