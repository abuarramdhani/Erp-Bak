...
class M_dpb extends CI_Model
{
    ...
    {
        $oracle = $this->load->database('oracle',true);
        $query = $oracle->query("WITH param AS (
            SELECT
                REGEXP_SUBSTR('$no_do' , '[^,]+', 1, LEVEL) p_item
            FROM
                dual
            CONNECT BY
                REGEXP_SUBSTR('$no_do' , '[^,]+', 1, LEVEL) IS NOT NULL)
            SELECT
            ...
            ,
                ...
                WHERE
                    ...
                    AND mmtt.subinventory_code = moqd.subinventory_code ) ) ATR ,
                (NVL(SUM(moqd.transaction_quantity), 0) - (
                SELECT
                    NVL(SUM(mr.RESERVATION_QUANTITY), 0)
                FROM
                ...
                WHERE
                    ...
                    AND msib3.organization_id = mmtt.organization_id
                    AND msib3.segment1 = msib.segment1
                    AND mmtt.subinventory_code = moqd.subinventory_code ) ) - tbl1.req_qty ATR_sisa
            FROM
                ...
                WHERE
                    ...
                    FROM
                        param)
                    --('3916224', '3916226') --(SELECT * FROM param)
