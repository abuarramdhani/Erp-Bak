...
class M_inputquestionnaire extends CI_Model {
	...
	public function GetQuestionnaireSegmentId($qe){
		$sql = "
			...
			where questionnaire_id='$qe' and segment_type=1
	...
	public function GetAttendant($id){
		$sql = "
			...
			where scheduling_id=18 and status=1";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	...
	public function GetQuestionnaire($id){
		$sql = "
			WITH
				submitted_questionnaire (questionnaire_id, submitted) AS (
 				select questionnaire_id, count(questionnaire_sheet_id) as submitted
 				from pl.pl_questionnaire_sheet
 				where scheduling_id='$id'
 				group by questionnaire_id)

			select a.questionnaire_id, a.questionnaire_title, case when b.submitted is NULL then 0 else b.submitted end as submitted
				from pl.pl_master_questionnaire a
				left join submitted_questionnaire b on a.questionnaire_id = b.questionnaire_id
		";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
