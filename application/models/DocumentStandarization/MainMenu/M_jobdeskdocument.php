...
class M_jobdeskdocument extends CI_Model
{
    ...
    public function ambilJobDescription($id = FALSE)
    {
        if ($id === FALSE) {
        $ambilJobDescription        = " select distinct     jd.jd_id as kode_jobdesc,
                                        ...
                                        from                ds.ds_jobdesk_detail as jddtl
                                                            join            ds.ds_jobdesk as jd
                                                                            on  jd.jd_id=jddtl.jd_id
    ...
    public function ambilDokumenJobDescription($id = FALSE)
    {
        if ($id === FALSE) 
        {
        ...
        $ambilDokumenJobDescription         = " select      jddtl.jd_id as kode_jobdesc,
                                                            jddtl.document_id as kode_dokumen,
                                                            jddtl.jd_detail_id as kode_detail_dokumen,
                                                            case split_part(jddtl.document_id, '-', 1)  when    'BP'
                                                                                                                then    (
                                                                                                                            select  bp.bp_file
                                                                                                                            from    ds.ds_business_process as bp
    ...
    public function deleteUnusedDocumentJD($jd_id, $jd_detail_id)
    {
        $deleteUnusedDocumentJD         = " delete from     ds.ds_jobdesk_detail
                                            where           jd_id=".$jd_id."
                                                            and     jd_detail_id not in (".$jd_detail_id.")";
        $this->db->query($deleteUnusedDocumentJD);
    }

    public function insertNewDocumentJD($data)
    {
        return $this->db->insert('ds.ds_jobdesk_detail', $data);
    }

    public function updateExistDocumentJD($data, $id)
    {
        $this->db->where('jd_detail_id', $id);
        $this->db->update('ds.ds_jobdesk_detail', $data);
    }    
}
