	...
	function getData($department, $month, $currentYear) {
		...
		if($kodeDepartment == 3) {
			$personalia = "
				union
				select
					kelompok_id as kelompok, target as target, sum(aktual) as aktual
				from (
					select
						data_kelompok.kelompok_id as kelompok_id, coalesce(data_target.target, '0') as target,
						count(tabel.*) as aktual
					from (
						select distinct
							nik, nama, kd_jabatan, jabatan, dept, bidang, unit, seksi
						from (
							select
								a.noind, nik, nama, c.kd_jabatan, c.jabatan, masukkerja, tglkeluar, keluar, b.*
							from
								hrd_khs.tpribadi a
					...
					where
						lower(rtrim(tabel.dept)) = lower('personalia')
		...
		return $this->personalia->query("
			select
				kelompok, target, sum(aktual) as aktual
			from (
				select
					kelompok_id as kelompok, target as target, sum(aktual) as aktual
				from (
					select
						data_kelompok.kelompok_id as kelompok_id,
					...
					from (
						...
						from (
							...
							left join hrd_khs.torganisasi c on
								a.kd_jabatan = c.kd_jabatan
							where
								((masukkerja <= cast('$currentYear-$month-30' as date))
					...
					left join hrd_khs.hr_target_efisiensi as data_target on
						...
						and data_target.kelompok_id = data_kelompok.kelompok_id
					where
						lower(rtrim(tabel.dept)) = lower('$department')
						and tabel.jabatan is not null
						and tabel.kd_jabatan is not null
						and data_kelompok.kelompok_id is not null
					group by
				...
				from (
					...
					from (
						select distinct
							nik, nama, kode_status_kerja, jabatan, dept, bidang, unit, seksi
						from (
							select
								a.noind, nik, nama, kode_status_kerja, c.jabatan, masukkerja, tglkeluar, keluar, b.*
							from
								hrd_khs.tpribadi a
							left join hrd_khs.tseksi b on
								a.kodesie = b.kodesie
