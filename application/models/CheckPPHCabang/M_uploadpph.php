...
class M_uploadpph extends CI_Model
{
	...
	function getBatch()
	{
		$sql = "SELECT max(batch_num) batch_num 
				FROM ap.ap_tax_data_cabang order by batch_num desc";
	...
	{
		$this->db->insert('ap.ap_tax_data_cabang',$data);
	}
	...
	function getDataUpload($batch = FALSE)
	{
		...
		$sql = "SELECT atd.batch_num
						...
						,atd.nama_file
						,atd.arsip
						, (SELECT count(*) jumlah FROM ap.ap_tax_data_cabang atd2 WHERE atd2.batch_num = atd.batch_num ) jumlah
				FROM ap.ap_tax_data_cabang atd $qbatch 
	...
	{
		$sql = "SELECT * FROM ap.ap_tax_data_cabang WHERE batch_num = '$no'";
		$query = $this->db->query($sql);
	...
	function getDataBatchSubtotal($batch)
	{
		$sql = "SELECT jenis_pph, tarif_pph, nama_vendor, no_npwp, no_invoice, max(tgl_transaksi) tgl_transaksi, bank, currency,SUM(dpp) dpp, SUM(pph) pph, jenis_jasa, lokasi, max(tgl_invoice) tgl_invoice
			FROM ap.ap_tax_data_cabang WHERE batch_num = '$batch' 
	...
	{
		$sql = "UPDATE ap.ap_tax_data_cabang SET 
					no_urut = '$newNoUrut'
	...
	{
		$sql = "UPDATE ap.ap_tax_data_cabang SET arsip = '1' WHERE batch_num = '$batch_num' ";
		$query = $this->db->query($sql);
	}
	...
	{
		$sql = "SELECT * FROM ap.ap_tax_data_cabang WHERE nama_vendor ='$vendor'  and no_invoice = '$invoice' and arsip = '1'";
		$query =$this->db->query($sql);
		return $query->num_rows();
	}
}