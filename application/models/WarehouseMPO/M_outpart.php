...
class M_outpart extends CI_Model {
  ...
  var $oracle;
    ...
    {
        $sql = "SELECT  
                  pov.vendor_name nama_subkon, 
                  knm.no_mobil,
                  mtrh.request_number no_spbs, 
                  mtrl.line_number,
                  gbh.batch_no NO_JOB, 
                  mtrh.creation_date tgl_spbs,
                  tmp.tgl_terima, tmp.tgl_mulai tgl_kirim, 
                  ...
                  ROUND
                  (AVG ((tmp.tgl_selesai - tmp.tgl_mulai) * 1440) OVER (PARTITION BY tmp.subinv, pov.vendor_name),2 ) average,
                CASE
        ...
        $sql = "SELECT  
                ...
                CASE
                  WHEN mtrl.line_status = 5
                    ...
                    ELSE 'X'
                 END keterangan,
                 TO_CHAR(mmt.transaction_date,'DD-MM-YYYY HH24:MI:SS') transaction_date
                FROM mtl_txn_request_headers mtrh,
                     mtl_txn_request_lines mtrl,
                     mtl_material_transactions_temp mmtt,
                     mtl_material_transactions mmt,
