...
public function getData()
	{
        ...
        $sql = "SELECT distinct
                        plla.PROMISED_DATE
                        ,TO_CHAR(plla.PROMISED_DATE, 'HH12:MI') PROMISED_TIME
                        ,msib.SEGMENT1 item
                        ,msib.DESCRIPTION
                        ,pha.SEGMENT1 no_po
                        ,pov.VENDOR_NAME
                        ...
                        ,hrl.LOCATION_CODE
                        ,ppf.FULL_NAME buyer
                        ,ppf1.FULL_NAME requester
						,plla.LINE_LOCATION_ID LINE
						,plla.GLOBAL_ATTRIBUTE14 status
                ...
                where pha.PO_HEADER_ID = pla.PO_HEADER_ID
                    ...
                    and ppf1.PERSON_ID = pda.DELIVER_TO_PERSON_ID
                    --
                --  and pha.SEGMENT1 = nvl(:P_PO,pha.SEGMENT1)
                    --
                    and nvl(pla.CANCEL_FLAG,'N') = 'N' 
                    and plla.CANCEL_DATE is null
                    and pla.CLOSED_CODE = 'OPEN'
                    and pla.CLOSED_DATE is null
                    and hrl.TOWN_OR_CITY = 'Kulonprogo'
                --  and hrl.LOCATION_CODE = 'KOM-DFG'
                --  and pda.DELIVER_TO_LOCATION_ID = 16103 -- Tuksono
                    and pha.PO_HEADER_ID not in (select pha.PO_HEADER_ID 
                                            from rcv_shipment_lines rsl
                                                ,po_headers_all pha
                                            where rsl.PO_HEADER_ID = pha.PO_HEADER_ID
                                                )
                    and msib.SEGMENT1 not like 'J%'
                    and plla.PROMISED_DATE >= sysdate - 3
					order by plla.PROMISED_DATE ASC,
					plla.GLOBAL_ATTRIBUTE14 asc";
                $query = $oracle->query($sql);
                return $query->result_array();
						}
						
	public function UpdatePromisedDate($line,$nopo,$prmdate){
		$oracle = $this->load->database('oracle',true);
		$sql = "UPDATE po_line_locations_all plla 
						SET plla.PROMISED_DATE = TO_TIMESTAMP('$prmdate','YYYY-MM-DD HH24:MI:SS' ) , plla.GLOBAL_ATTRIBUTE14 ='Y'
						WHERE plla.LINE_LOCATION_ID = (SELECT plla.LINE_LOCATION_ID 
																					FROM po_line_locations_all plla ,po_distributions_all pda, po_headers_all pha
						WHERE pda.LINE_LOCATION_ID = plla.LINE_LOCATION_ID
						and pha.PO_HEADER_ID = plla.PO_HEADER_ID
						and pha.SEGMENT1 = '$nopo'
						and plla.LINE_LOCATION_ID = '$line')";
		$query=$oracle->query($sql);

	}
		
	public function UpdateStatus($line,$nopo){
		$oracle = $this->load->database('oracle',true);
		$sql = "UPDATE po_line_locations_all plla 
						SET plla.GLOBAL_ATTRIBUTE14 ='Y'
						WHERE plla.LINE_LOCATION_ID = (SELECT plla.LINE_LOCATION_ID 
																					FROM po_line_locations_all plla ,po_distributions_all pda, po_headers_all pha
						WHERE pda.LINE_LOCATION_ID = plla.LINE_LOCATION_ID
						and pha.PO_HEADER_ID = plla.PO_HEADER_ID
