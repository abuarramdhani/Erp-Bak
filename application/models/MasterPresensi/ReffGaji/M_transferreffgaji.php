...
class M_transferreffgaji extends CI_Model
{
	...
	public function getDataCetakPerKodesie($periode){
		// $sql = "select reff.*,sie.seksi, sie.unit, sie.dept 
		// 		FROM \"Presensi\".TReffGaji reff 
		// 		INNER JOIN hrd_khs.TSeksi sie 
		// 		ON reff.kodesie = sie.kodesie 
		// 		WHERE to_char(reff.tanggal,'mmyy') ='$periode' 
		// 		AND reff.jns_transaksi in('01') 
		// 		ORDER BY left(reff.noind,1),reff.kodesie, reff.noind";
		$sql = "select * 
				from ( 
	...
	public function getDataCetakPerKodeInduk($periode){
		$sql = "select reff.*,sie.seksi, sie.unit, sie.dept,pri.sekolah,pri.jurusan,pri.pendidikan 
				FROM \"Presensi\".TReffGaji reff 
				INNER JOIN hrd_khs.TSeksi sie 
				ON reff.kodesie = sie.kodesie 
				inner join hrd_khs.tpribadi pri 
				on pri.noind = reff.noind
				WHERE to_char(reff.tanggal,'mmyy') ='$periode' 
				AND reff.jns_transaksi in('01') 
				ORDER BY reff.noind";
		return $this->personalia->query($sql)->result_array();
