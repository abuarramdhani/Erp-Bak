    ...
    class M_rekappembayaranjht extends CI_Model
    {
        ...
        function get_jht_year($no_ind, $year)
        {
            ...
            $sql = "
                ...
                SELECT sys.sys_month_names.nama AS bulan,
                    ...
                    COALESCE(tabel_transaksi_karyawan.trf_jht_kary, '0') AS tarif_karyawan,
                    COALESCE(CAST(NULLIF(tabel_transaksi_karyawan.jht_kary, '') AS NUMERIC), 0) AS jht_karyawan,
                    COALESCE(tabel_transaksi_karyawan.trf_jht_prshn, '0') AS tarif_perusahaan,
                    COALESCE(CAST(NULLIF(tabel_transaksi_karyawan.jht_prshn, '') AS NUMERIC), 0) AS jht_perusahaan,
                    (COALESCE(CAST(NULLIF(tabel_transaksi_karyawan.jht_kary, '') AS NUMERIC), 0)
                        + COALESCE(CAST(NULLIF(tabel_transaksi_karyawan.jht_prshn, '') AS NUMERIC), 0)) AS total_jht
        ...
        function get_sum($no_ind, $year)
        {
            $sql = "
                ...
                SELECT 
                    SUM(COALESCE(CAST(NULLIF(tabel_transaksi_karyawan.jht_kary, '') AS NUMERIC), 0)) AS jht_karyawan,
                    SUM(COALESCE(CAST(NULLIF(tabel_transaksi_karyawan.jht_prshn, '') AS NUMERIC), 0)) AS jht_perusahaan,
                    SUM(COALESCE(CAST(NULLIF(tabel_transaksi_karyawan.jht_kary, '') AS NUMERIC), 0)
                        + COALESCE(CAST(NULLIF(tabel_transaksi_karyawan.jht_prshn, '') AS NUMERIC), 0)) AS total_jht
                FROM sys.sys_month_names
                LEFT JOIN tabel_transaksi_karyawan
                ON sys.sys_month_names.id = EXTRACT(MONTH FROM tabel_transaksi_karyawan.tanggal)
