...
class M_shipmentmonitoringsystem extends CI_Model
{
  ...
  function __construct()
  {
    ...
    public function ambilDO($no_ship)
    {
      $db = $this->load->database();
      $sql = "select no_do from ex.shipment_line where shipment_header_id = '$no_ship'";
      $runQuery = $this->db->query($sql);
      return $runQuery->result_array();
    }
    ...
    public function getShpById($id)
    {
       ...
       $sql = "select 
                    ...
                    sh.estimate_loading_date,
                    sh.ship_from_id,
                    sh.ship_to_province_id,
                    sh.ship_to_city_id,
                    sh.ship_to_address,
                    sh.estimate_loading_date,
                    sh.actual_loading_date,
                    sh.actual_depart_date,
                    sh.actual_arrive_date,
                    sh.creation_date,
                    sh.created_by,
                    sh.ship_to_cabang_id,
                    sh.is_full_flag,
                    sh.full_percentage,
                    sh.pr_number,
                    sh.pr_line_number,
                    sh.vehicle_type_id,
                    vh.volume_cm3
                    from ex.shipment_header sh
                    left join ex.vehicle vh on vh.vehicle_id = sh.vehicle_type_id
...
--              and sh.actual_loading_date is null
    ...
    public function getSavingMe($fingo_tb,$tujuan_tb,$namabarang_tb,$provinsi_tb,$quantity_tb,$kota_tb,$alamat_tb)
    {
      $db = $this->load->database();
      $sql = "insert into ex.entrusted_goods 
              (goods_id, 
              quantity, 
              creation_date, 
              ship_to_cabang_id, 
...
// -------------------------------------------------------------------------------------------------
       // update header
       public function updateheaderSMS($no_ship,$usrname,$estimasi,$estimasi_loading,$finish_good,$status,$cabang,$kendaraan,$full_persen,$alamat,$provinsi,$koota,$pr_numb,$pr_linee)
       {
          $db = $this->load->database();
          $sql = "
                update 
...
// -----------------------setup----------------------------------------------------------------------------------------------
    ...
    {
        $db = $this->load->database();
        $sql = "delete from ex.shipment_header where shipment_header_id = '$id'";
        $runQuery1 = $this->db->query($sql);
        $sql2 = "delete from ex.shipment_line where shipment_header_id = '$id'";
