<?php
class M_ImportFile extends CI_Model {
    ...
    public function __construct()
    {
	 ...
	 function updateData($org_id,$sub_inv,$kode_assy,$type_assy,$kode_item,$locator,$alamat_simpan,$lppbmokib,$picklist,$username_save){
	    ...
	    $sql ="update khs.khslokasisimpan set ALAMAT_SIMPAN ='$alamat_simpan' , LPPB_MO_KIB ='$lppbmokib' ,  PICKLIST ='$picklist', LAST_UPDATED_BY ='$username_save' ,LAST_UPDATE_DATE = sysdate where ORGANIZATION_ID ='$org_id' and $a2 and $b2 and $c2 and $d2 ";
	    $query = $this->db->query($sql);
	    return;
	  }

    function insertData($org_id,$sub_inv,$kode_assy,$type_assy,$kode_item,$locator,$alamat_simpan,$lppbmokib,$picklist,$username_save){
    $sql="insert INTO KHS.KHSLOKASISIMPAN (ORGANIZATION_ID, SUB_INV, ASSEMBLY, ASSEMBLY_TYPE, COMPONENT, LOCATOR, ALAMAT_SIMPAN, LPPB_MO_KIB, PICKLIST, CREATED_BY, CREATION_DATE) VALUES ('$org_id', '$sub_inv', '$kode_assy', '$type_assy', '$kode_item', '$locator', '$alamat_simpan', '$lppbmokib', '$picklist', '$username_save', sysdate)";
    $query = $this->db->query($sql);
    return;
  }
