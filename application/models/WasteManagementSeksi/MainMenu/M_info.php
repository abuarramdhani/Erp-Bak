...
class M_info extends CI_Model
{
    ...
    public function chartSeksi($id,$tanggal){
        ...
        where kirim.id_jenis_limbah = '$id' and to_char(kirim.tanggal_kirim, 'M Y') = to_char(cast('1 ".$tanggal."' as date),'M Y')  and status_kirim = '1' 
        group by seksi.section_name order by sum(cast(kirim.berat_kirim as integer)) desc;";
        $result = $this->db->query($query);
        return $result->result_array();
    }

    public function chartLimbah($sie,$tanggal){
        ...
        where kirim.kodesie_kirim = '$sie' and to_char(kirim.tanggal_kirim, 'M Y') = to_char(cast('1 ".$tanggal."' as date),'M Y')  and status_kirim = '1' 
