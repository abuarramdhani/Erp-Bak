<?php
class M_monitoringloadingproduksi extends CI_Model {
	...
	public function dataLP($dept, $plan_id, $jam){
		$sql = "with detail as
    (
    select distinct
           ffvv.FLEX_VALUE                                                          cost_center
    ...
    --      ,bores.OPERATION_SEQUENCE_ID
          --
          ,KHSFUNCLOADODM.hitung_seninkamis(bores.RESOURCE_ID,bores.OPERATION_SEQUENCE_ID) senin_kamis
          ,KHSFUNCLOADODM.hitung_jumatsabtu(bores.RESOURCE_ID,bores.OPERATION_SEQUENCE_ID) jumat_sabtu
          ,KHSFUNCLOADODM.hitung_average(bores.RESOURCE_ID,bores.OPERATION_SEQUENCE_ID) average
          ,KHSFUNCLOADODM.hitung_pembagi(bores.RESOURCE_ID,bores.OPERATION_SEQUENCE_ID) pembagi
          --
          ,nvl(
          (
           select sum(coalesce(md.DAILY_DEMAND_RATE, md.USING_REQUIREMENT_QUANTITY)) qty
             from msc_system_items msi
                 ...
                 ,msc_plans mp
                 ,mfg_lookups ml
            where msi.SR_INVENTORY_ITEM_ID = bor.ASSEMBLY_ITEM_ID
    ...
    --          and msi.ORGANIZATION_ID =:P_IO_ID
              ...
              and last_day(to_date('01'||to_char(sysdate,'MON-YY')))
    ...
    --  and br.RESOURCE_CODE in ('MA-L-GCG1R','MA-L-GCG1L')
    )
    select dd.*
          ,round (dd.qty / dd.pembagi,2) need_shift
          ,round (gg.sigma,2) sigma_shift
          ,round ((round (gg.sigma,2) / '$jam'),2) need_hour
    from detail dd
        ,(select sum (sigma) sigma
                ,cc.resource_code
            from
          (select distinct
                 dd.qty/dd.pembagi sigma
                ,dd.resource_code
            from detail dd
           ) cc
           group by cc.resource_code
         ) gg
    where dd.resource_code = gg.resource_code
