...
class C_DataKlaimPekerjaKeluar extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model('PayrollManagement/DataKlaimPekerjaKeluar/M_dataklaimpekerjakeluar');
	...
	public function index()
    {
        ...
        $this->load->view('PayrollManagement/DataKlaimPekerjaKeluar/V_index', $data);
	...
	public function Check(){
		...
		if($checkPeriode){
			...
			$checkAvailNoind	= $this->M_dataklaimpekerjakeluar->checkAvailNoind();
			...
			$this->load->view('V_Header',$data);
			$this->load->view('V_Sidemenu',$data);
			$this->load->view('PayrollManagement/DataKlaimPekerjaKeluar/V_check_master', $data);
			$this->load->view('V_Footer',$data);
		}
	}
		...
		$date			= date('Y-m-d');
		$getDataGajianPersonalia	= $this->M_dataklaimpekerjakeluar->getDataGajianPersonalia($varYear,$varMonth,$date);
		$no = 0;
		foreach($getDataGajianPersonalia as $row1){
			...
			$r_kp = 0;
			$r_tkp = 0;
			$getKp	= $this->M_dataklaimpekerjakeluar->getKp($noind,$varYear,$varMonth);
			foreach($getKp as $row){
				$r_kp		= $row->tambahan;
				$r_desc_kp	= $row->desc_;
			}
			
			$getTkp	= $this->M_dataklaimpekerjakeluar->getTkp($noind,$varYear,$varMonth);
			foreach($getTkp as $row){
				$r_tkp		= $row->tambahan;
				$r_desc_tkp	= $row->desc_;
			}
			
			$getTransaksiPajak	= $this->M_dataklaimpekerjakeluar->getTransaksiPajak($varYear,$noind);
			foreach($getTransaksiPajak as $row){
				$r_gaji_sd_bln_tsb	= $row->penghasilan_sd_bln_tsb;
			}
		
			$getMasterPtkp			= $this->M_dataklaimpekerjakeluar->getMasterPtkp($stat_pajak,$jt_pajak,$date);
			foreach($getMasterPtkp as $row){
				$fx_ptkp	= $row->ptkp_per_tahun;
				$fx_stat_pajak	= $row->status_pajak;
			}
			$fx_cicilan_hutang = 0;
			$getHutang	= $this->M_dataklaimpekerjakeluar->getHutang($noind,$varMonth,$varYear);
			foreach($getHutang as $row){
				$fx_cicilan_hutang = $fx_cicilan_hutang + $row->jumlah_transaksi;
			}
			
			$getPekerjaSakit	= $this->M_dataklaimpekerjakeluar->getPekerjaSakit($bln_sakit);
			foreach($getPekerjaSakit as $row){
				$p_sakit = $row->persentase;
			}
			
			$getParameterPPh	= $this->M_dataklaimpekerjakeluar->getParameterPPh();
			// PERHITUNGAN KODE STATUS  " B "
					if(substr($noind,0,1)=="B" or substr($noind,0,1)=="D" or substr($noind,0,1)=="J" or substr($noind,0,1)=="G"){
						...
						// INSERT PEMBAYARAN PENGGAJIAN
						$this->M_dataklaimpekerjakeluar->insert($data);
						$data_asuransi = array(
						...
						$this->M_dataklaimpekerjakeluar->insert_asuransi($data_asuransi);
						...
						$this->M_dataklaimpekerjakeluar->insert_transaksi_pajak($data_pajak);
						...
						$this->M_dataklaimpekerjakeluar->insert_transaksi_insentif_kemahalan($data_insentif_kemahalan);
						//REVIEW PERHITUNGAN 
						// echo "<table>";
