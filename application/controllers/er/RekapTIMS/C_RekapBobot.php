...
class C_RekapBobot extends CI_Controller {
	...
	public function tampilkanData()
	{
		...
		$masakerja  = $this->M_rekaptims->ambilDataMasa($noind);
		$tgl = $masakerja[0]['masukkerja'];
		$year_in = substr($tgl, 0, 4);
		$month_in = substr($tgl, 5, 2);
		$day_in = substr($tgl, 8, 2);

		$year_now = date('Y');
		$month_now = date('m');
		$day_now  = date('d');

		$data['masakerja'] = array(
								'tahun' => $year_now-$year_in,
								'bulan' => $month_now-$month_in,
								'hari' => $day_now-$day_in,
							);
	...
	public function ExportRekapDetailExcel()
	{
		...
		$masakerja  = $this->M_rekaptims->ambilDataMasa($NoInduk);
		$tgl = $masakerja[0]['masukkerja'];
		$year_in = substr($tgl, 0, 4);
		$month_in = substr($tgl, 5, 2);
		$day_in = substr($tgl, 8, 2);
		...
		$bulan = $month_now-$month_in;
		$hari  = $day_now-$day_in;
		$masakerjaa = "$tahun tahun $bulan bulan $hari hari";
		...
		foreach ($rekap_all as $rekap_data) {
			...
			$worksheet->setCellValue('D'.$highestRow, $masakerjaa);
