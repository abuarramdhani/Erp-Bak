...
class C_TagihanExternal extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['Title'] = 'Tagihan External';
		$data['Menu'] = 'Tagihan External';
		$data['SubMenuOne'] = '';
		$data['SubMenuTwo'] = '';
		$data['Filter'] = 'Pure';
		...
		$data['allTagihan'] = $this->M_external->getTagihan();

		$this->load->view('V_Header', $data);
		$this->load->view('V_Sidemenu', $data);
		$this->load->view('MonitoringFlowOut/Master/V_TagihanExternal', $data);
	...
	public function search()
	{
		...
		$data['allTagihan'] = $this->M_external->filterCar($newtxtTglMFO1, $newtxtTglMFO2, $slcSeksiFAjx);

		if (empty($data['allTagihan'])){
			echo "<center><h3 style='color:red;'>Data tidak ditemukan</h3></center>";
		} else {
		return $this->load->view('MonitoringFlowOut/Master/V_ViewResultTagExt', $data);
		}
	}
	...
	public function ExportExt() {
		...
		if ($tglAwal){
			$dataSelected = $this->M_external->filterCar(
				date('Y-m-d', strtotime($tglAwal)),
				date('Y-m-d', strtotime($tglAkhir)),
				$seksi
			);
		} else {
			$dataAll = $this->M_external->getTagihan();
		}
		...
		//Final
		$worksheet->setTitle('Tagihan External');
		...
		header('Content-Disposition: attachment;filename="REKAPITULASI FLOW OUT EXTERNAL BELUM KIRIM CAR ' . date('d-m-Y') . '.xls"');
