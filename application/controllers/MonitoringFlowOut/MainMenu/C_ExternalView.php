...
class C_ExternalView extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['Title'] = 'External View';
		$data['Menu'] = 'External View';
		$data['SubMenuOne'] = '';
		$data['SubMenuTwo'] = '';
		...
		$data['dats'] = $this->M_external->getExternal();
		$data['seksi'] = $this->M_master->getSeksi();
		$data['onlyseksi'] = $this->M_master->getSeksi2();
		$data['getAllIns'] = $this->M_external->getAllInsp();
		...
		$this->load->view('MonitoringFlowOut/External/V_ExtView', $data);
		$this->load->view('V_Footer', $data);
	}

	public function getQi($id)
	{
		$data = $this->M_external->getEditInsp($id);
	...
	public function search()
	{
		...
		$data['dats'] = $this->M_external->sortByDate($newtxtTglMFO1, $newtxtTglMFO2, $slcSeksiFAjx);
		if (empty($data['dats'])){
			echo "<center><h3 style='color:red;'>Data tidak ditemukan</h3></center>";
		} else {
		return $this->load->view('MonitoringFlowOut/External/V_ViewResult', $data);
		}
	}
	...
	function edit($id)
	{
		...
		$data['seksi'] = $this->M_master->getSeksi();
		$data['getEdit'] = $this->M_external->getOne($id);
		$data['getEdIns'] = $this->M_external->getEditInsp($id);
		$data['id'] = $id;

		$this->load->view('V_Header', $data);
		$this->load->view('V_Sidemenu', $data);
		$this->load->view('MonitoringFlowOut/External/V_ExtEdit', $data);
		$this->load->view('V_Footer', $data);
	}
	...
	function update($id)
	{
		...
		$this->M_external->updateExternal($id, $dataExt);
	...
	public function DataGrafikMFO()
	{
		...
		$data['perSeksi'] = $this->M_external->disBySeksi($monthGraf, $yearGraf);
	...
	public function ExportExt() {
		...
		if ($tglAwal){
			$dataSelected = $this->M_external->sortByDate(
				date('Y-m-d', strtotime($tglAwal)),
				date('Y-m-d', strtotime($tglAkhir)),
				$seksi
			);
		} else {
			$dataAll = $this->M_external->getExternal();
		}
		...
		$worksheet->getColumnDimension('G')->setWidth(5); //Qr
		$worksheet->getColumnDimension('H')->setWidth(5); //Car
		$worksheet->getColumnDimension('I')->setWidth(20); //Seksi PJ
		$worksheet->getColumnDimension('J')->setWidth(20); //Vendor
		...
		$worksheet->setCellValue('A1', 'REKAPITULASI FLOW OUT EXTERNAL');
		...
		$worksheet->setCellValue('G5', 'QR');
		$worksheet->setCellValue('H5', 'CAR');
		$worksheet->setCellValue('I5', 'SEKSI PJ');
		$worksheet->setCellValue('J5', 'VENDOR');
		...
		if ($tglAwal){
			for ($i=0; $i < sizeof($dataSelected); $i++) { 
				...
				if (empty($dataSelected[$i]['upload_qr'])){
					$worksheet->setCellValue('G' . $highestRow, '×')->getStyle('G' . $highestRow)->applyFromArray($MidCenter);
				} else {
					$worksheet->setCellValue('G' . $highestRow, '✓')->getStyle('G' . $highestRow)->applyFromArray($MidCenter);
				}
				...
				$worksheet->setCellValue('I' . $highestRow, $dataSelected[$i]['seksi_penanggungjawab'])->getStyle('I' . $highestRow)->applyFromArray($cont);
				$worksheet->setCellValue('J' . $highestRow, $dataSelected[$i]['vendor'])->getStyle('J' . $highestRow)->applyFromArray($cont);
			...
			for ($i=0; $i < sizeof($dataAll); $i++) { 
				...
				$worksheet->setCellValue('I' . $highestRow, $dataAll[$i]['seksi_penanggungjawab'])->getStyle('I' . $highestRow)->applyFromArray($cont);
				$worksheet->setCellValue('J' . $highestRow, $dataAll[$i]['vendor'])->getStyle('J' . $highestRow)->applyFromArray($cont);
		...
		$bottomsheet = $highestRow + 8;
			//Bottom Styling
		$worksheet->getStyle('F' . $bottomsheet . ':I' . ($bottomsheet + 6))->applyFromArray(array(
			'alignment' => array(
				'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
				'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER
			),
		...
		$worksheet->setCellValue('I' . $bottomsheet, 'Yogyakarta, .............................');
		$worksheet->setCellValue('I' . ($bottomsheet + 1), 'Dibuat,');
		$worksheet->setCellValue('I' . ($bottomsheet + 3), '............................');
		$worksheet->setCellValue('I' . ($bottomsheet + 4), 'Kepala Seksi Madya QI A');

		//Final
		$worksheet->setTitle('External');
		...
		header('Content-Disposition: attachment;filename="REKAPITULASI FLOW OUT EXTERNAL ' . date('d-m-Y') . '.xls"');
