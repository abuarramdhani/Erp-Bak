...
class C_NonConformityAndroid extends CI_Controller
{
	...
	public function cetakPOLandscape($request_id)
	{
		...
		$pdf = new mPDF('utf-8', array(320,215), 0, '',3, 3, 53, 6, 3, 3);
		// $pdf       = new mPDF('utf-8','A4-P', 0, '', 3, 3, 3, 10, 3, 3);
		$filename = 'tes.pdf';

		$data['head'] = $this->M_nonconformity->cetakPOLandscape($request_id);

		// echo'<pre>';
		// print_r($data);

		$pdf->setHTMLHeader($this->load->view('PurchaseManagementGudang/titip/V_headerCetak',$data, TRUE));
		$pdf->setHTMLFooter($this->load->view('PurchaseManagementGudang/titip/V_footerCetak',$data, TRUE));
		$pdf->writeHTML($this->load->view('PurchaseManagementGudang/titip/V_contentCetak',$data, TRUE));
