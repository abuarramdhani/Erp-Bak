...
class C_RequestKasie extends CI_Controller
{
    ...
    public function viewrequest(){
		...
		if ($name == 'approveAskanit' || $name == 'approvePPC' || $name == 'approveCabang') {
		...
		foreach ($getdata as $key => $val) {
			...
			if ($ket == 'finished') { //tabel finished
				if ($status[1] == $status[3]) { // jml item request == jml item yg direject -> reject semua
					...
					if ($name == 'finishedKasie') { // resp. Miscellaneous Kasie -> kalau seksi login = seksi data, simpan datanya
						$seksi[0]['seksi'] == $val['seksi'] ? array_push($datanya, $getdata[$key]) : '';
					}else {
						array_push($datanya, $getdata[$key]);
					}
				}else {
					if ($bts_status == 8) { // data sudah input oracle dari input costing
	...
	public function batasStatus($status){
		if ($status == 'Proses Approve Ka. Cabang / Showroom') {
			$bts = 1;
	...
	public function SaveRequest(){
		...
		if (!empty($item)) {
			...
			for ($i=0; $i < count($item) ; $i++) { 
				...
				$saveitem = $this->M_request->saveItem($idheader, $iditem, $jenis[$i], $ket_cost[$i], $cost_center[$i], $inv_item[$i], $item[$i], $desc[$i],
