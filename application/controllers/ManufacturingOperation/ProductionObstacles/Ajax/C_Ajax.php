...
class C_Ajax extends CI_Controller
{
	...
	{	
		...
		$type = 'n';
		$kategori = 'n';
		$data = $this->M_ajax->selectInduk($term,$type,$kategori);
		echo json_encode($data);
	}

		public function select2CetakInduk()
	{	
		$term= $this->input->post('term');
		$type = $this->input->post('type');
	...
	{	
		...
		$data = $this->M_ajax->selectCabang($induk,$term);
		echo json_encode($data);
	}
	...
	public function deleteHambatan()
	{
		$id = $this->input->post('id');
		$data = $this->M_ajax->deleteHam($id);
		echo json_encode($data);
		
	}

	public function deleteHambatanNon()
	{
		$id = $this->input->post('id');
		$data = $this->M_ajax->deleteHamNon($id);
		echo json_encode($data);
	...
	public function searchHambatan()
	{
		...
		$type = $this->input->post('type');
		$kategori = $this->input->post('kategori');
		$data = $this->M_ajax->searchHambatan($tgl1,$tgl2,$type,$kategori);
		$x=1;
		foreach ($data as $k) {
			echo '<tr>
				<td>'.$x.'</td>
	...
	public function searchHambatanNon()
	{
		$tgl1 = $this->input->post('tgl1');
		$tgl2 = $this->input->post('tgl2');
		$type = $this->input->post('type');
		$data = $this->M_ajax->searchHambatanNon($tgl1,$tgl2,$type);
		$x=1;
		foreach ($data as $k) {
			echo '<tr>
				<td>'.$x.'</td>
	...
	public function exportHamMesin()
	{
		...
		$kategori = $this->input->post('kategoriHambatan');
		...
		$data = $this->M_ajax->reportHambatan($tgl1,$tgl2,$type,$kategori);
		...
		$worksheet->setCellValue('A1', 'Laporan Hambatan produksi Cetakan '.$type);
		$worksheet->setCellValue('A3', 'Hambatan Mesin tanggal '.$tgl1.' s/d '.$tgl2);
		$worksheet->setCellValue('A5', 'Kategori : '.$kategori);
	...
	public function exportHamNonMesin()
	{
		$tgl1 = $this->input->post('tgl_hambatan1');
		$tgl2 = $this->input->post('tgl_hambatan2');
		$type = $this->input->post('typeCetak');
		...
		$data = $this->M_ajax->reportHambatanNon($tgl1,$tgl2,$type);
		...
		// $worksheet->getStyle("A1:L1")->getFont()->setSize(18);
		$worksheet->setCellValue('A1', 'Laporan Hambatan produksi Cetakan '.$type);
		$worksheet->setCellValue('A3', 'Hambatan Non Mesin tanggal '.$tgl1.' s/d '.$tgl2);
