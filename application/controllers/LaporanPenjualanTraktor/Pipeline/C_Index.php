...
class C_Index extends CI_Controller
{
	...
	public function index(){
		...
		$validate_detail = $this->M_pipeline->check_data_availability('khs_lpb_penjualan_detail', 'REQUEST_ID', $max_req_detail);
	...
	public function support_data($data, $table, $pk){
		...
		if ($table == 'khs_lpb_skip_date'){
			$ora_skip = $this->M_pipeline->get_all_skip_date('ORACLE');
			$ora_skip = array_column($ora_skip, "DATE_ID");

			$pgr_skip = $this->M_pipeline->get_all_skip_date('POSTGRES');
			$pgr_skip = array_column($pgr_skip, "DATE_ID");

			$diff = array_diff($pgr_skip, $ora_skip);

			if (!(empty($diff))){
				$key = array_keys($diff)[0];
				$this->M_pipeline->delete_data($table, "DATE_ID", $diff[$key]);
			}
		}
