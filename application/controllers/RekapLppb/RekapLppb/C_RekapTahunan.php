...
class C_RekapTahunan extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('RekapLppb/RekapLppb/M_rekaptahunan');
	...
	public function index()
	{
		...
		$this->load->view('RekapLppb/RekapLppb/V_RekapTahunan', $data);
	...
	public function searchTahunan()
	{
		...
		for ($i=0; $i <count($navbulan) ; $i++) { 
			$prmbulan = $navbulan[$i]['mon'];
			$hasil = $this->M_rekaptahunan->getSelisih($prmbulan, $io);
			if ($hasil != null) {
				$data['selisih'][$i] = $hasil['0']['SELISIH'];
                $data['bulan'][$i] = $navbulan[$i]['mon'];
				$data['no'][$i] = $i;
        ...
        $this->load->view('RekapLppb/RekapLppb/V_TblRekapTh', $data);
    }
}