...
class C_DatabaseQTW extends CI_Controller
{
    ...
    public function findPemandu()
    {
        ...
        $loker = $_GET['loker_slc'] == 'Tuksono' ? '02' :($_GET['loker_slc'] == 'Pusat' ? '01': '');
    ...
    public function findDataPDF()
    {
        ...
        $namain = $jenis == 1 ? 'Tahun '.date('Y', strtotime($_GET['tahun'])) : (date('M Y', strtotime($_GET['start'])) == date('M Y', strtotime($_GET['end'])) ? 'Bulan '.date('M Y', strtotime($_GET['start'])) : 'Bulan '.date('M Y', strtotime($_GET['start']))).' - '.date('M Y', strtotime($_GET['end']));
