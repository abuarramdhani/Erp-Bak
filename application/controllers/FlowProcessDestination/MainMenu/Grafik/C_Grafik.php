	...
	function __construct()
		{
			...
			$this->load->model('FlowProcessDestination/M_Grafik');
	...
	public function index()
		{
			...
			$this->load->view('FlowProcessDestination/MainMenu/Grafik/V_Index',$data);
			$this->load->view('V_Footer',$data);
			$this->load->view('FlowProcessDestination/MainMenu/Grafik/V_Chart',$data);
		}
			...
			$recapComp = array();
				...
				if ($getComp) {
					foreach ($getComp as $key => $value) {
						$recapComp[$key]['tgl'] = date('Y-m-d',strtotime($value['creation_date']));
						$recapComp[$key]['cd_comp'] = $value['drw_code'];
						$recapComp[$key]['desc_prod'] = $value['product_description'];
					}
				}
				$data['recapDay'] = $recapDay;
				$data['ke'][0]['ke'] = $ke;
				$data['recapComp'] = $recapComp;
			// $this->load->view('FlowProcessDestination/MainMenu/Grafik/V_Chart2',$data);
