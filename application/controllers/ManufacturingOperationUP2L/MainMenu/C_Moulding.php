...
class C_Moulding extends CI_Controller
{
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('component_code', 'txtComponentCodeHeader', 'required');
		$this->form_validation->set_rules('production_date', 'txtProductionDateHeader', 'required');
		$this->form_validation->set_rules('txtMouldingQuantityHeader', 'txtMouldingQuantityHeader', 'required');
	...
	public function read($id)
	{
		...
		for ($i=0; $i < count($mould); $i++) { 
			$employee[] = array('name' 	=>$mould[$i]['name'],
		...
		if(count($scrap)>=1){
			...
			$scrap2[] = array('type_scrap' => $scrap[$i]['type_scrap'] ,
		...
		if(count($bongkar)>=1){
			...
			$bongkar2[] = array('quantity' => $bongkar[$i]['qty'],
								'jumlah' => $bongkar[$i]['jumlah'],
								'no'	=> $i+1,
						 );
			}
		}else{
			$bongkar2[] = array('quantity' => '-',
								'jumlah' => '-',
								'no' => '-'
						 );
		}
		...
		$head[] = array('component_code' =>$mould[0]['component_code'],
...
/* Location: ./application/controllers/ManufacturingOperationUP2L/MainMenu/C_Moulding.php */
