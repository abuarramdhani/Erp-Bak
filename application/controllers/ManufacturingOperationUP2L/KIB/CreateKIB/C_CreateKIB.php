...
class C_CreateKIB extends CI_Controller
{
	...
	function __construct()
	{
		parent::__construct();
		 $this->load->helper('form');
			...
			$this->load->model('ManufacturingOperationUP2L/InventoryKIB/M_createkib');
	...
	public function pdf1($status,$no_batch,$kib)
	{
		...
		// $html = $this->load->view('ManufacturingOperationUP2L/CreateKIB/V_SO',$data,true);
	...
	public function pdf3($nomorset)
	{
		...
		$filename			= 'KIB_'.time().'.pdf';
		$html = $this->load->view('ManufacturingOperationUP2L/CreateKIB/V_Pdf3',$data,true);
		// echo "$html";
		// -------------- update flag
	...
	public function printpdf2($serial){
		...
		$filename			= 'KIB_'.time().'.pdf';
		$html = $this->load->view('ManufacturingOperationUP2L/CreateKIB/V_Pdf_Serial',$data,true);
		$pdf->WriteHTML($html,0);
		$pdf->Output($filename, 'I');
...
public function printpdf99(){
	...
	$temp_filename = array();
		...
		$filename			= 'KIB_'.time().'.pdf';
		$html = $this->load->view('ManufacturingOperationUP2L/CreateKIB/V_Pdf_Serial99',$data,true);
		$pdf->WriteHTML($html,0);
		$pdf->Output($filename, 'I');
	...
	public function printpdf($org,$status,$no_batch,$kib,$n){
		...
		foreach ($dataKIB as $key => $value) {
			...
			if (in_array($value['REQUEST_NUMBER'], $arrayREQNUM)) {
				...
				$dataKIBKelompok[$indexnya]['PLANED_DATE'] = $value['PLANED_DATE'];
				$dataKIBKelompok[$indexnya]['TO_LOCATOR_CODE'] = $value['TO_LOCATOR_CODE'];
				$dataKIBKelompok[$indexnya]['OPERATOR'] = $tampung_operator;
		...
		$filename			= 'KIB_'.time().'.pdf';
		if ($org == 'odm') {
			$html = $this->load->view('ManufacturingOperationUP2L/CreateKIB/V_Odm',$data,true);
		} else {
			$html = $this->load->view('ManufacturingOperationUP2L/CreateKIB/V_Pdf',$data,true);
		}
		$pdf->WriteHTML($html,0);
		$pdf->Output($filename, 'I');
