...
class C_DraftCuti extends CI_Controller
{
	...
	{ //to del cuti / call by ajax
		$this->M_permohonancuti->deleteCuti($_POST['id_cuti']);
	}

	public function Request()
	{ //to request cuti /call by ajax
		$id = $_POST['id_cuti'];
		$noind = $this->session->user;
		...
		$thread2 = array(
			'lm_pengajuan_cuti_id' => $id_cuti,
			'status' => '1',
			'detail' => '(Set Approver) - Surat Permohonan Cuti ini telah dimintakan approve kepada ' . $getapp1['0']['nama'] . ' ' . $app2 . ' ' . $hubker,
			'waktu' => $time
		);
	...
	public function PreviewCetak($id_cuti)
	{ // preview for Hari Perkiraan Lahir Letter :v
