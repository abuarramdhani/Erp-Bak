 <?php
	...
	class C_Approval extends CI_Controller
	{
		...
		public function index()
		{
			...
			//special icon for Seksi Hubker login on approval
			if (strstr($kodesie, '4010101')) {
				$data['user'] = "<span class='label label-success'><i class='fa fa-database'> Hubungan Kerja</i></span>";
				$data['count_inprocess'] = count($this->M_approval->getHubker('1'));
				$data['count_approved']  = count($this->M_approval->getHubker('2'));
				$data['count_rejected']  = count($this->M_approval->getHubker('3'));
			} else {
				$data['user'] = '';
		...
		public function Inprocess()
		{
			...
			if (strstr($kodesie, '4010101')) { //this just user have that kodesie Hubker (kasie)
				$data['Inprocess'] = $this->M_approval->getHubker($status);
			} else {
		...
		public function Approved()
		{
			...
			if (strstr($kodesie, '4010101')) {
				$data['Approved'] = $this->M_approval->getHubker($status);
			} else {
		...
		public function Rejected()
		{
			...
			if (strstr($kodesie, '4010101')) {
				$data['Rejected'] = $this->M_approval->getHubker($status);
			} else {
		...
		public function Canceled()
		{
			...
			if (strstr($kodesie, '4010101')) {
				$data['Canceled'] = $this->M_approval->getHubker($status);
			} else {
		...
		public function All()
		{
			...
			if (strstr($kodesie, '4010101')) {
				$data['All'] = $this->M_approval->getHubker($status);
			} else {
		...
		public function Approve($id)
		{
			...
			$threadHubker = array( //if cuti need approval from Hubker -> insert this thread
				'lm_pengajuan_cuti_id' => $id_cuti,
				'status' => '0',
				'detail' => '(Waiting Approval) - Menunggu Approval dari seksi Hubungan Kerja',
				'waktu'  => $time
			);
			...
			$mailAddressHubker = $this->M_approval->getMail('hubker')->result_array();
			...
			if ($approve == '2') {
				if ($level == '1' && !strstr("02,03,04", $kdjabatanLv1)) {
					...
					if (strstr($nextAppr, $dataCuti['0']['jenis']) && $level == '2') {
						for ($i = 0; $i < count($mailAddressHubker); $i++) {
							$this->sendMail($mailAddressHubker[$i]['internal_mail'], 'Seksi Hubungan Kerja', $object, $linkApproval, $keperluan, $jenis); //call send mail function
						}
					}
				}
			}
