...
class C_Civil extends CI_Controller
{
	...
	{
		$user = $this->session->user;
		$user_real_name = $this->session->employee;

		$data  = $this->general->loadHeaderandSidemenu('Civil Maintenance', 'Input Order', 'Order', 'Input Order', '');

		$jenisOrder = $this->M_civil->getTableJenisOrder();
		$detailPekerja = $this->M_civil->getDetailPkj($user)[0];

		$data['judul'] = $this->M_civil->listJnsPkj();
		$data['claz'] = '';
		$data['jenisOrder'] = $jenisOrder;

		// default value
		$data['user_default'] = $user;
		$data['user_name_default'] = $user_real_name;
		$data['seksi_default'] = $detailPekerja['seksi'];
		$data['lokasi_kerja_default'] = $detailPekerja['lokasi_kerja'];

		if (in_array($user, ['T0007', 'B0720', 'B0560']))
			$data['claz'] = 'cmo_slcJnsOrder';

		// admin -> bisa ubah tanggal input
		$isAdmin = false;
		$listOfSuperUser = ['B0825'];

		if (in_array($user, $listOfSuperUser)) {
			$isAdmin = true;
		}
	...
	{
		$config = array(
			'upload_path' => './assets/upload/CivilMaintenance/Thread/',
