...
class C_PekerjaKeluar extends CI_Controller
{
	...
	function exportExcel(){
		...
		$getPekerjaKeluar = $this->M_datapekerja->getPekerjaKeluar($tanggal_awal,$tanggal_akhir,$kdLokasi);
		if($getPekerjaKeluar == FALSE){
		...
		$worksheet->getStyle('A5:J5')->applyFromArray($thead);
		$worksheet->getStyle('A5:J5')->applyFromArray($borderleft);
		...
		$worksheet->getStyle('J5')->applyFromArray($borderright);
		$worksheet->getStyle('A5:J5')->applyFromArray($bordertop);
		$worksheet->getStyle('A5:J5')->applyFromArray($borderbottom);
		...
		$sheet->setCellValue('A1','DATA PEKERJA KELUAR CV. KARYA HIDUP SENTOSA');
		...
		$sheet->setCellValue('D5','TGL.KELUAR');
		...
		$sheet->setCellValue('J5','SEBAB KELUAR');

		$i = 5;
		$nomor = 0;
		foreach ($getPekerjaKeluar as $key => $data) {
			$i++;
			$nomor++;
			...
			$sheet  ->setCellValueExplicit($kolomA, $nomor, PHPExcel_Cell_DataType::TYPE_NUMERIC)
					...
					->setCellValueExplicit($kolomD, date('Y-m-d',strtotime($data->tglkeluar)), PHPExcel_Cell_DataType::TYPE_STRING)
					...
					->setCellValueExplicit($kolomJ, rtrim($data->sebabklr), PHPExcel_Cell_DataType::TYPE_STRING)
					;
			...
			$worksheet->getStyle($kolomA.':'.$kolomJ)->applyFromArray($tdata);
		...
		header("Content-Disposition: attachment; filename=RekapDataPekerjaKeluar_".str_replace(' ', '-', $bulan).".xls");
	...
	function preview(){
		...
		if($getPekerjaKeluar == FALSE){
			...
			$data['dataPekerja'] = $getPekerjaKeluar;
			$data['bulan']		= $bulan;
