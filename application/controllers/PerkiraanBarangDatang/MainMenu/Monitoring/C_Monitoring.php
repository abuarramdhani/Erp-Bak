...
class C_Monitoring extends CI_Controller
{
	...
	function __construct()
		{
			...
			$this->load->model('PerkiraanBarangDatang/M_monitoring');
	...
	public function index()
		{
			...
			$data['value'] = $this->M_monitoring->getData();
			date_default_timezone_set("Asia/Jakarta");
			$data['date'] = date('d F Y H:i:s');

			$this->load->view('V_Header',$data);
            $this->load->view('V_Sidemenu',$data);
            $this->load->view('PerkiraanBarangDatang/MainMenu/Monitoring/V_View',$data);
	...
	public function UpdatePromisedDate(){
		$line = $this->input->post('line');
		$nopo = $this->input->post('nopo');
		$prmdate1 = $this->input->post('prmdate');
		$prmtime = $this->input->post('prmtime');
		$prmdate2 = $prmdate1.' '.$prmtime;
		$prmdate3= date("d-m-Y H:i:s",strtotime($prmdate2));
		$prmdate= date("Y-m-d H:i:s",strtotime($prmdate3));
		// echo '<pre>';
		// print_r($line);
		// print_r($nopo);
		// print_r($prmdate2);
		...
		// print_r($prmtime);
		// exit;
		if ( !$this->M_monitoring->UpdatePromisedDate($line,$nopo,$prmdate)){
	...
	public function UpdateStatus(){
		$line = $this->input->post('line');
		$nopo = $this->input->post('nopo');
		if ( !$this->M_monitoring->UpdateStatus($line,$nopo)){
			echo json_encode('success');
		}else{
			echo json_encode('error');
		}
	...
	function DeptClass()
	{
		...
		$data = $this->M_monitoring->selectDeptClass($term);
	...
	function DeptCode()
	{
		...
		$data = $this->M_monitoring->selectDeptCode($deptclass);
		echo json_encode($data);
	}
	...
	public function searchData()
	{
		...
		//$datadt = $this->M_monitoring->searchdata($available_op,$hari_kerja,$input_parameter,$deptclass,$deptcode,$monthPeriode);
		$data = array('dataform' =>  $this->M_monitoring->searchdata($available_op,$hari_kerja,$input_parameter,$deptclass,$deptcode,$monthPeriode2),
	...
	public function savetable(){
		...
		foreach ($tabledata as $kunci) {
			$cek = $this->M_monitoring->cektabel($deptclass, $kunci['RESOURCES'], $monthPeriode, $kunci['ITEM_CODE']);
			//  echo'<pre>'; print_r(sizeof($cek)); exit;
			if(count($cek) > 0){
				$this->M_monitoring->update(array(
				...
				$this->M_monitoring->saveData(array(
					'RESOURCES'=> $kunci['RESOURCES'],
					'RESOURCE_DESC' => $kunci['RESOURCE_DESC'],
					'ITEM_CODE' => $kunci['ITEM_CODE'],
					'ITEM_DESCRIPTION' => $kunci['ITEM_DESCRIPTION'],
