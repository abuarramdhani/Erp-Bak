			...
			processResults: function (data) {
				return {
					results: $.map(data, function (obj) {
						return {
							id: obj.kodesie,
							text: obj.kodesie + " - " + obj.daftar_tseksi,
						};
					}),
				};
			},
				...
				if (noindPers.length) {
					...
					noindErp.each(function () {
						...
						var txtPers = $(".Pers" + i)
							...
							.text();
						if (txtPers !== txtErp) {
							if (txtPers.length < 1 && txtErp.length > 1) {
								// pers kosong dan erp ada
								arry.push(str + "|insert");
								$(this).css("background-color", "#8bc34a");
								$(".Pers" + i)
									.eq(x)
									.css("background-color", "#8bc34a");
							} else if (txtPers.length > 1 && txtErp.length > 1) {
								// pers ada dan erp ada{
	...
	if (stat == "reject") {
		Swal.fire({
			...
			window.location.replace(
				baseurl + "PolaShiftSeksi/Approval/ApprovalShift"
			);
		});
	} else {
		Swal.fire({
...
function init_select_tukar() {
	...
	$(".pss_noind_to_all").change(function () {
		...
		$.ajax({
			...
			success: function (response) {
				if (typeof response === "string") alertNoShift(response, ini);
				else setPerKolomAll(response);
			},
			error: function (jqXHR, textStatus, errorThrown) {
				console.log(textStatus, errorThrown);
			},
		...
		Swal.fire({
			...
			if (result.value) {
				...
				$.ajax({
					...
					success: function (response) {
						window.location.replace(
							baseurl + "PolaShiftSeksi/Approval/ApprovalTukarShift"
						);
					},
					error: function (jqXHR, textStatus, errorThrown) {
						alert("b");
...
function setPerKolom(response, ini) {
	if (response.length < 1) {
		...
		ini
			.closest("div.pss_data_pkj")
			.find(".pss_nama")
			.val(response[0][0]["nama"]);
		$(".pss_selc2").attr("disabled", false);
		$("#surat-loading").attr("hidden", true);
	}
	var mx = $(".pss_tgl").length / 2;
	for (let i = 0; i < mx; i++) {
		ini
			...
			.find(".pss_tgl")
			.eq(i)
			.val(response[i][0]["tanggal"]);
		ini
			.closest("div.pss_data_pkj")
			.find(".pss_shift")
			.eq(i)
			.val(response[i][0]["shift"]);
		ini
			.closest("div.pss_data_pkj")
			.find(".kd_sift")
			.eq(i)
			.val(response[i][0]["kd_shift"]);
	}
}
...
function simpan_to_table() {
	...
	var oke = 1;
	if (noin == null || noin.length < 1) {
		alert("Harap Isi Noind!");
		return false;
	}
	$(".pss_gs").each(function () {
		if ($(this).val().length < 1) {
			oke = 0;
			alert("Harap Isi Semua Shift !");
			return false;
		}
	});
	if (oke == 0) {
