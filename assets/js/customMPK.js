...
$(document).ready(function () {
      ...
      for (var i = 0; i < pekerja.length; i++) {
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            $("#tblMPKSimForkliftTambahPekerja .slcMPKSimForkliftGantiJenis")
              .last()
              .select2();

            $("#tblMPKSimForkliftTambahPekerja .txtMPKSimForkliftMulaiBerlaku")
              .last()
              .datepicker({
                autoclose: true,
                todayHiglight: true,
            ...
            $("#tblMPKSimForkliftTambahPekerja .txtMPKSimForkliftAkhirBerlaku")
              .last()
              .datepicker({
                autoclose: true,
                todayHiglight: true,
  ...
  $("#tblMPKSimForkliftTambahPekerja").on(
    "click",
    ".btnMPKSimForkliftHapusPekerja",
    function () {
      $(this).closest("tr").remove();
    }
  );
  ...
  $("#btnMPKSimForkliftSimpanPekerja").on("click", function () {
    ...
    if (tr_all && tr_all.length > 0) {
      $("#ldgMPKSimForkliftTambahLoading").show();
      $("#tblMPKSimForkliftTambahPekerja")
        .find(".btnMPKSimForkliftHapusPekerja")
        .hide();
      $("#tblMPKSimForkliftTambahPekerja")
        .find(".spnMPKSimForkliftSimpanPekerjaSukses")
        .hide();
      $("#tblMPKSimForkliftTambahPekerja")
        .find(".spnMPKSimForkliftSimpanPekerjaGagal")
        .hide();
      $("#tblMPKSimForkliftTambahPekerja")
        .find(".imgMPKSimForkliftSimpanPekerja")
        .show();
      ...
      $("#tblMPKSimForkliftTambahPekerja")
        ...
        .attr("disabled", "disabled");
      var diproses = 0;
      $("#tblMPKSimForkliftTambahPekerja tbody tr").each(function (
        index,
        element
      ) {
...
function checkSimForkliftChecked(tblMPKSimForkliftList) {
  ...
  if (jumlah_checked > 1 && jumlah_checked <= 20) {
    ...
    // console.log(id_checked);
    var url_param = $.param(id_checked);
    $("#btnSimForkliftCetakPdf").attr(
      "href",
      baseurl + "MasterPekerja/SimForklift/cetakpdf?" + url_param
    );
    $("#btnSimForkliftCetakPdf").attr("disabled", false);
    $("#btnSimForkliftCetakImg").attr(
      "href",
      baseurl + "MasterPekerja/SimForklift/cetakimg?" + url_param
    );
    $("#btnSimForkliftCetakImg").attr("disabled", false);
    $("#btnSimForkliftCetakCrl").attr(
      "href",
      baseurl + "MasterPekerja/SimForklift/cetakcrl?" + url_param
    );
    $("#btnSimForkliftCetakCrl").attr("disabled", false);
  } else {
    $("#btnSimForkliftCetakPdf").attr("disabled", true);
