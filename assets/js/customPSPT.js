$((_) => {
  ...
  const datePSPTDay =
    String(datePSPT.getDate()).length < 2
      ? "0" + String(datePSPT.getDate())
      : String(datePSPT.getDate());

  const datePSPTMonth =
    String(datePSPT.getMonth()).length < 2
      ? "0" + String(datePSPT.getMonth())
      : String(datePSPT.getMonth());
  ...
  const swalPSPTAjaxConfirmation = (
    type,
    text,
    url,
    data,
      ...
      Swal.fire({
        ...
        if (result.value) {
          $.ajax({
            ...
            contentType: ((_) => {
              if (isFormData === "yes") return false;
            })(),
            processData: ((_) => {
              if (isFormData === "yes") return false;
            })(),
  ...
  const tooltipPSPTDestroy = (selector) => {
    return new Promise((response) => {
      $(selector)
        .removeAttr("title data-html data-placement")
        .tooltip("destroy");
      setTimeout((_) => {
        response(true);
      }, 250);
  ...
  const setPSPTRegisterFieldValue = (
    type = null,
    name = null,
    workerStats = null,
    section = null,
  ...
  $("#txtPSPTIdentityNumber").on({
    blur: function () {
      ...
      if (
        userId.length === 5 &&
        userId.includes("P") === false &&
        userId.includes("K") === false
      ) {
        $.ajax({
          beforeSend: (_) => {
          ...
          .done((resp) => {
            if (resp.length != 0) {
              ...
              tooltipPSPTValidation(
                ...
                "Data dengan nomor induk ini tidak ditemukan."
              );
              swalPSPTToastrAlert(
                "error",
                `Data dengan nomor induk ${userId} tidak ditemukan.`
              );
              setPSPTRegisterFieldValue();
            }
          })
        ...
        (userId.length === 5 && userId.includes("P")) ||
        userId.includes("K")
      ) {
        tooltipPSPTDestroy("#txtPSPTIdentityNumber").then((_) => {
          tooltipPSPTValidation(
            "#txtPSPTIdentityNumber",
            "Untuk pekerja OS pendampingan dilakukan oleh perusahaan masing-masing"
          );
          setPSPTRegisterFieldValue();
          checkPSPTEmptyField(".divPSPTRegisterFormField", "input");
        });
        ...
        tooltipPSPTDestroy("#txtPSPTIdentityNumber").then((_) => {
          tooltipPSPTValidation(
            "#txtPSPTIdentityNumber",
            "Data yang anda tulis belum sesuai"
          );
          setPSPTRegisterFieldValue();
          checkPSPTEmptyField(".divPSPTRegisterFormField", "input");
        });
  ...
  $(".btnPSPTRegister").on("click", function () {
    ...
    if ($(".has-error").length === 0 && $("#txtPSPTTRN").val() != "-") {
      ...
      let type = "question";
      let text =
        "<h4>Anda sudah yakin dengan informasi yang anda berikan?</h4>";
      ...
      swalPSPTAjaxConfirmation(type, text, url, data, success, fail).then(
        (resp) => {
          if (resp.message === "fail" && resp.data === undefined) {
            ...
            resp.message === "fail" &&
            resp.data.status === "already available"
          ) {
            $("#mdlPSPTAlertRegister")
              .find(".modal-title")
              .html('<i class="fa fa-warning"></i> Peringatan');
            $("#mdlPSPTAlertRegister")
              .find("p")
              .html(
                `Nomor induk <b>${$(
                  "#txtPSPTIdentityNumber"
                ).val()}</b> sebelumnya telah melakukan pendaftaran dengan detail nomor pendaftaran sebagai berikut.`
              );
            ...
            $("#mdlPSPTAlertRegister")
              .find(".modal-title")
              .html('<i class="fa fa-info"></i> Informasi');
            $("#mdlPSPTAlertRegister")
              .find("p")
              .html(
                "Pendaftaran sukses dilakukan dengan detail nomor pendaftaran sebagai berikut."
              );
            $("#txtPSPTRegisterId").val(resp.data.registered_number);
            $("#mdlPSPTAlertRegister").modal("show");
            setPSPTRegisterFieldValue("refresh");
      ...
      swalPSPTToastrAlert(
        "error",
        "Silahkan mengisi informasi yang masih kosong."
      );
    }
  });
  ...
  $("#slcPSPTDataWorkerStats, #slcPSPTDataWorkLocation").on(
    "change",
    function () {
      dataTablePSPTList
        .columns(13)
        .search($("#slcPSPTDataWorkerStats").val())
        .columns(2)
        .search($("#slcPSPTDataWorkLocation").val())
        .draw();
    }
  );
    ...
    if ($(".has-error").length === 0) {
      ...
      swalPSPTAjaxConfirmation(
        ...
        if (resp.message === "fail" && resp.data === undefined) {
          ...
          resp.message === "fail" &&
          resp.data.detail === "wrong data"
        ) {
          tooltipPSPTValidation(
            "#filePSPTImportExcel",
            "File yang anda berikan tidak sesuai. Silahkan melakukan export ulang terlebih dahulu."
          );
        } else if (
          resp.message === "fail" &&
          resp.data.detail === "data not equal"
        ) {
          tooltipPSPTValidation(
            "#filePSPTImportExcel",
            "Data pada file ini telah usang. Silahkan melakukan export ulang terlebih dahulu."
          );
        } else if (
          resp.message === "fail" &&
          resp.data.detail === "wrong file format"
        ) {
          tooltipPSPTValidation(
            "#filePSPTImportExcel",
            "File ini tidak dapat diimport. Silahkan cek kembali file yang akan anda import."
          );
        } else if (resp.message === "success") {
          setTimeout((_) => {
            location.reload();
      ...
      swalPSPTToastrAlert(
        "error",
        "Silahkan memilih file excel terlebih dahulu."
      );
    }
  });
    ...
    swalPSPTAjaxConfirmation(type, text, url, data, success, fail).then(
      (resp) => {
        if (resp.message === "success") {
          dataTablePSPTList.rows($(this).parents("tr")).remove().draw();
          $("#tblPSPTList")
            .find("td:first-child")
            .each(function (key) {
              $(this).html(key + 1);
...
$("#txtPSPTIdentityNumber").on("change", function () {
  var val = $(this).val().split("");
  if (val[0] != "A") {
    Swal.fire({
      type: "error",
