/**
 * REFACTORING ALERT
 * TOLONG AGAR MEMBUAT NAMA VARIABEL YANG JELAS, PREFER KE ENGLISH LANGUAGE, JANGAN DISINGKAT
 *
 * (THIS CODE NOT ONLY MAINTED BY YOURSELF)
 *
...
$(document).ready(function () {
	...
	setTimeout(() => {
		$(".input_Pekerja_SP").on("ifChecked", function () {
			let jenis = $(this).val();
			let kelas = $("#slc_Kelas_SP").val(); // UNUSED VARIABLE
		...
		if (jenis == "" || jenis == null) {
			...
			$.ajax({
				...
				success: function (result) {
					...
					$(".input_Pekerja_SP").iCheck("uncheck");
					$("#slc_pri_pkj_SP").val("").trigger("change");
					$("#kelas_pkj_SP").attr("hidden", Outsorcing != "1");
					...
					const hasEmailAccount = result.pkj[0].email_internal ? true : false;
					const hasPidginAccount = result.pkj[0].pidgin_account ? true : false;

					// This is bad way
					// this should be rendered from backend
					// because this is too difficult for maintenance
					$("#plus_pkj_SP").html(
						`
						<div class="box">
						<form class="form-horizontal" id="addReadonlyForm">
										...
										<div class="form-group">
											...
											<div class="col-lg-8">
												...
												<p style="color: red; display: inline-block">
													<i>*) Beri tanda centang apabila No. Telp sama dengan No. HP</i>
												</p>
											</div>
										</div>
									</div>
										...
										<form class="form-horizontal" id="form_email_account">
											<div class="form-group">
												<label class="control-label col-lg-4">
													${hasEmailAccount ? "" : '<input type="checkbox" name="create_email_account" />'}
												</label>
												<div class="col-lg-8">
											...
											<div class="form-group">
												...
												<div class="col-lg-8">
													<input class="form-control" type="text" name="email_address" value="${hasEmailAccount ? result.pkj[0].email_internal : ""}" disabled />
											...
											<div class="form-group">
												...
												<div class="col-lg-8">
													<input class="form-control" type="text" name="email_display_name" disabled />
												</div>
											</div>
											<div class="form-group">
												<label class="control-label col-lg-4">Password</label>
												<div class="col-lg-8">
													<input class="form-control" placeholder="Default is 123456" type="password" value="123456" name="email_password" disabled />
								...
								<div class="col-lg-6">
									<div class="col-lg-12">
										<form class="form-horizontal" id="form_pidgin_account">
											<div class="form-group">
												<label class="control-label col-lg-4">
												${hasPidginAccount ? "" : '<input type="checkbox" name="create_pidgin_account" />'}
												</label>
												<div class="col-lg-8">
											...
											<div class="form-group">
												...
												<div class="col-lg-8">
													<input class="form-control" placeholder="nama_email@chat.quick.com" value="${hasPidginAccount ? result.pkj[0].pidgin_account : ""}" type="text" name="pidgin_username" disabled />
											...
											<div class="form-group">
												<label class="control-label col-lg-4">Nama</label>
												<div class="col-lg-8">
													<input class="form-control" type="text" name="pidgin_name" disabled />
												</div>
											</div>
											<div class="form-group">
												<label class="control-label col-lg-4">Password</label>
												<div class="col-lg-8">
													<input class="form-control" placeholder="Default is 123456" type="password" value="123456" name="pidgin_password" disabled />
													<small>* Default: 123456</small>
												</div>
											</div>
										</form>
							...
							<div class="box-body panel-footer text-center">
								<button type="button" class="btn btn-success btn_Save_SP" name="button">
          ...
          `
					...
					$(".btn_Save_SP").on("click", function () {
						const DataNew = {
							...
							// EMAIL
							create_email_account: $("input[name=create_email_account]").is(":checked"),
							email_address: $("input[name=email_address]").val(),
							email_display_name: $("input[name=email_display_name]").val(),
							email_password: $("input[name=email_password]").val(),

							// PIDGIN
							create_pidgin_account: $("input[name=create_pidgin_account]").is(":checked"),
							pidgin_username: $("input[name=pidgin_username]").val(),
							pidgin_name: $("input[name=pidgin_name]").val(),
							// pidgin_email: $("input[name=pidgin_email]").val(), // now is disabled, may be will used in future
							pidgin_password: $("input[name=pidgin_password]").val(),
						};
						...
						if (!$("#slc_RuangLingkup_SP").val()) {
							return swal.fire("Warning", "Ruang Lingkup belum diisi", "warning");
						}
	...
	$.ajax({
		...
		success: (res) => {
			...
			for (let i = 0; i < res.length; i++) {
				...
				isi += `<option ${selek} value = "${res[i].kodesie + "|" + res[i].kd_jabatan}" > ${res[i].kodesie + "|" + res[i].jabatan}</option > `;
			}
