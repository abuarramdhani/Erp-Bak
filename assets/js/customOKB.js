$(document).ready(function () {
  ...
  $(".btnOKBStatusOrderNormal").on("click", function () {
    ...
    $(
      ".slcAlasanUrgensiOKB+.select2-container>.selection>.select2-selection"
    ).attr({
    ...
    $(".nbdOKB").each(function () {
      ...
      if ($(this).val() != "") {
        ...
        if (
          tahunBaru == 1 ||
          (tahunBaru == 0.5 && bulanBaru == 1) ||
          (tahunBaru == 0.5 && bulanBaru == 0.5 && tanggalBaru == 1) ||
          (tahunBaru == 0.5 && bulanBaru == 0.5 && tanggalBaru == 0.5)
        ) {
  ...
  $(document).on("change", ".slcOKBNewOrderList", function () {
    ...
    if (setup_item != "BELUM DI SET") {
      ...
      var html =
        '<option value="' + inv_item_id + '">' + ItemName + "</option>";

      prn.find(".slcOKBNewOrderListNamaBarang").append(html);
      prn
        .find(".slcOKBNewOrderListNamaBarang")
        .val(inv_item_id)
      ...
      var html =
        '<input type="hidden" class="form-control" name="cutoff[]" id="" value="' +
        cutoff_terdekat +
        '"></input>';
  ...
  $(document).on("change", ".slcOKBNewOrderListNamaBarang", function () {
    ...
    if (setup_item != "BELUM DI SET") {
      ...
      var html =
        '<option value="' + inv_item_id + '">' + itemkode + "</option>";

      prn.find(".slcOKBNewOrderList").append(html);
      prn
        .find(".slcOKBNewOrderList")
        .val(inv_item_id)
        .trigger("change.select2");
  ...
  $(document).on("change", ".nbdOKB", function () {
    ...
    var estArrival = $(this)
      .parentsUntil(".tblOKBOrderNew")
      .find(".hdnEstArrivalOKB")
      .html();
    // alert(estArrival);
    var leadtime = $(this)
      .parentsUntil(".tblOKBOrderNew")
      .find(".leadtimeOKB")
      .val();
    var urgentReasontxa = $(this)
      .parentsUntil(".tblOKBOrderNew")
      .find(".txaOKBNewOrderListUrgentReason");
    var urgentReason = $(this)
      .parentsUntil(".tblOKBOrderNew")
      .find(
        ".slcAlasanUrgensiOKB+.select2-container>.selection>.select2-selection"
      );
    var urgentReason1 = $(this)
      .parentsUntil(".tblOKBOrderNew")
      .find(".slcAlasanUrgensiOKB");
    var urgentFlag = $(this)
      .parentsUntil(".tblOKBOrderNew")
      .find(".hdUrgentFlagOKB");
      ...
      if (susulan != "Y") {
        ...
        if (
          ...
          Swal.fire({
            ...
            title: "Peringatan",
            text:
              "Order ini berstatus urgent, Silahkan mengisi alasan urgensi !",
          });
          $(this).attr("style", "background-color : #ff00004d; width:250px;");
          urgentReasontxa.attr("required");
          urgentReasontxa.removeAttr("readonly");
  ...
  $(".checkBoxConfirmOrderOkebaja").on("ifChecked ifUnchecked", function (
    event
  ) {
  ...
  $(".btnOKBNewOrderList").click(function () {
    ...
    var html =
      '<div class="col-md-12 formOKB" id-form="' +
      (LastDataRow + 1) +
      '">' +
      ...
      "Order No : " +
      (LastDataRow + 1) +
      ' <button type="button" class="btn btn-success btnOKBNewOrderListCancel" title="clear line" data-row="' +
      (LastDataRow + 1) +
      ...
      "<th>Nama Barang</th>" +
      "<th>:</th>" +
      '<td><select class="select2 slcOKBNewOrderListNamaBarang slcOKBOrder" id="slcOKBNewOrderListNamaBarang' +
      (LastDataRow + 1) +
      '" name="" required style="width:360px"></td>' +
      "</tr>" +
      "<tr>" +
      "<th>Deskripsi</th>" +
      "<th>:</th>" +
      '<td><textarea style="height: 34px; width:360px;" class="form-control txaOKBNewOrderDescription" id="txaOKBNewOrderDescription' +
      (LastDataRow + 1) +
      '" onkeyup="changeDescNewOrder(' +
      (LastDataRow + 1) +
      ')" name="txtOKBinputDescription[]" readonly></textarea></td>' +
      "</tr>" +
      "<tr>" +
      "<th>Quantity</th>" +
      "<th>:</th>" +
      ...
      "<th>:</th>" +
      "<td>" +
      '<select class="select2 slcAlasanUrgensiOKB" style="width: 250px;"' +
      disab +
      ">" +
      "<option></option>" +
      ...
      '<input type="file" name="fileOKBAttachment' +
      (LastDataRow + 1) +
      '[]" style="display: inline-block;">' +
      '<button type="button" class="btn btn-primary ml-3 btnOKBAddInputAttachment" data-row="' +
      (LastDataRow + 1) +
      '" style="display: inline-block;"><i class="fa fa-plus"></i></button>' +
      "</li>" +
      "</td>" +
      "</tr>" +
      ...
      '<label class="control-label"+ <h4> <img style="width:30px" src="' +
      baseurl +
      'assets/img/gif/loading5.gif"> <b>Sedang Mengambil Data ...</b></h4> </label>' +
      "</div>" +
      "</center>" +
  ...
  $(".checkAllApproveOKB").on("ifChecked ifUnchecked", function (event) {
    ...
    $(".checkApproveOKB").on("ifChanged", function (event) {
      if (
        $(".checkApproveOKB").filter(":checked").length ==
        $(".checkApproveOKB").length
      ) {
        // alert('benith my skins');
  ...
  $(document).on("click", ".btnOKBListOrderHistory", function () {
    let orderid = $(this)
      .parentsUntil("tbody")
      .find(".tdOKBListOrderId")
      .html();
    $(".mdlOKBListOrderHistory-" + orderid).modal("show");
    if (
      $(".divOKBListOrderHistory-" + orderid)
        .find("span:first")
        .html() == ""
    ) {
      ...
      $.ajax({
        type: "post",
        url:
          baseurl + "OrderKebutuhanBarangDanJasa/Requisition/getHistoryOrder",
        ...
        success: function (resp) {
          ...
          $(resp).each(function (index, value) {
            ...
            if (value.JUDGEMENT == "A") {
              ...
              if (
                value.NATIONAL_IDENTIFIER == $("#txtOKBOrderRequestorId").val()
              ) {
                var explain =
                  "Anda telah memberi keputusan Approved pada order ini ";
                if (
                  value.ITEM_DESCRIPTION_BEFORE &&
                  value.ITEM_DESCRIPTION_AFTER
                ) {
                  explain +=
                    "dan telah mengubah deskripsi item dari <b>" +
                    value.ITEM_DESCRIPTION_BEFORE +
                    "</b> menjadi <b>" +
                    value.ITEM_DESCRIPTION_AFTER +
                    "</b> ";
                  ...
                  if (value.QUANTITY_BEFORE && value.QUANTITY_AFTER) {
                    explain +=
                      "kemudian mengubah quantity item dari <b>" +
                    ...
                    if (
                      value.ORDER_PURPOSE_BEFORE &&
                      value.ORDER_PURPOSE_AFTER
                    ) {
                      explain +=
                        "dan mengubah alasan order dari <b>" +
                        value.ORDER_PURPOSE_BEFORE +
                        "</b> menjadi <b>" +
                        value.ORDER_PURPOSE_AFTER +
                        "</b>";
                    ...
                    explain +=
                      "kemudian mengubah alasan order dari <b>" +
                      value.ORDER_PURPOSE_BEFORE +
                      "</b> menjadi <b>" +
                      value.ORDER_PURPOSE_AFTER +
                      "</b>";
                  ...
                  explain +=
                    "dan telah mengubah quantity item dari <b>" +
                    value.QUANTITY_BEFORE +
                    " " +
                    value.UOM +
                    "</b> menjadi <b>" +
                  ...
                  explain +=
                    "dan telah mengubah alasan order dari <b>" +
                    value.ORDER_PURPOSE_BEFORE +
                    "</b> menjadi <b>" +
                    value.ORDER_PURPOSE_AFTER +
                    "</b>";
                ...
                var explain =
                  value.FULL_NAME +
                  " telah memberi keputusan Approved pada order ini ";
                if (
                  value.ITEM_DESCRIPTION_BEFORE &&
                  value.ITEM_DESCRIPTION_AFTER
                ) {
              ...
              if (
                ...
                var explain =
                  "Anda telah memberi keputusan Rejected pada order ini dengan alasan " +
                  value.NOTE +
                  ".";
              } else {
                var explain =
                  value.FULL_NAME +
                  " telah memberi keputusan Rejected pada order ini dengan alasan " +
                  value.NOTE +
                  ".";
              }
            } else {
              ...
              if (
                ...
                var explain =
                  "Approval order ini sedang diproses oleh " +
                  value.FULL_NAME +
                  ".";
              }
            }
            if (index == 0) {
                ...
                .append(
                  "<p>\
                    ...
                    '</b>\
                                    <span class="' +
                    order_class +
                    '"><label class="control-label"><i class="' +
                    icon_class +
                    '"></i><b> ' +
                    status +
                    " </b></label> - " +
              ...
              index > 0 &&
              resp[index - 1].JUDGEMENT != null &&
              resp[index - 1].JUDGEMENT != "R"
            ) {
              $(".divOKBListOrderHistory-" + orderid)
                .find("span:first")
                .append(
  ...
  $(document).on("click", ".btnOKBListRequisitionHistory", function () {
    ...
    if (
      $(".divOKBListRequisitionHistory-" + orderid)
        .find("span:first")
        .html() == ""
    ) {
      ...
      $.ajax({
        ...
        url:
          baseurl +
          "OrderKebutuhanBarangDanJasa/Pengelola/getHistoryRequisition",
        ...
        if (dismiss != "close") {
          if (reason == null || reason == "") {
            ...
            $.ajax({
              type: "POST",
              url:
                baseurl + "OrderKebutuhanBarangDanJasa/Approver/ApproveOrder",
              data: {
                orderid: orderid,
                judgment: judgement,
                person_id: person_id,
          ...
          if (dismiss != "close") {
            if (reason == null || reason == "") {
              ...
              $.ajax({
                ...
                url:
                  baseurl +
                  "OrderKebutuhanBarangDanJasa/Approver/rejectOrderAfterApproved",
                data: {
                  order_id: order_id,
                  person_id: person_id,
                  note: reason,
  ...
  $(document).on("click", ".btnOKBPengelolaAct", function () {
    ...
    if (orderClass == "") {
      ...
      $.ajax({
        type: "POST",
        url:
          baseurl + "OrderKebutuhanBarangDanJasa/Pengelola/TindakanPengelola2",
        data: {
          orderid: orderid,
      ...
      $.ajax({
        type: "POST",
        url:
          baseurl + "OrderKebutuhanBarangDanJasa/Pengelola/TindakanPengelola",
        data: {
          orderid: orderid,
          orderClass: orderClass,
          person_id: pengelola,
  ...
  $(document).on("click", ".btnOKBReleaseOrderPulling", function () {
    ...
    var cutOffDateValues = cutOffDateNodes
      .map(function (i, e) {
        return $(e).html();
      })
      .toArray();
  ...
  $(document).on("click", ".btnOKBDetailReleasedOrder", function () {
    ...
    $.ajax({
      type: "POST",
      url:
        baseurl + "OrderKebutuhanBarangDanJasa/Puller/ShowDetailReleasedOrder",
      data: {
        pre_req_id: orderid,
      },
      success: function (response) {
        ...
        if (dismiss != "close") {
          if (reason == null || reason == "") {
            ...
            $.ajax({
              type: "POST",
              url:
                baseurl + "OrderKebutuhanBarangDanJasa/Purchasing/ApproveOrder",
              data: {
                pre_req_id: orderid,
                judgement: judgement,
                person_id: person_id,
  ...
  $(document).on("click", ".btnOKBDetailApprovedOrderPurchasing", function () {
    ...
    $.ajax({
      ...
      url:
        baseurl +
        "OrderKebutuhanBarangDanJasa/Purchasing/ShowDetailOrderApproved",
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response == null || response == "") {
            html =
              '<center><span><i class="fa fa-warning">No Data Found</i></span></center>';
          } else {
            html = response;
          }
        ...
        success: function (response) {
          ...
          for (let i = 0; i < response.length; i++) {
            ...
            if (elm["FILE_NAME"] == null) {
              html +=
                '<span><i class="fa fa-warning"></i>Tidak ada attachment</span><br>';
            } else {
              ...
              if (ext == "pdf") {
                ...
                html +=
                  ...
                  elm["ADDRESS"] +
                  elm["FILE_NAME"] +
                  '" target="_blank" data-toggle="tooltip" title="Tipe File ini adalah PDF. Klik untuk melihat isinya." style="font-size: 18px;">' +
                  elm["FILE_NAME"] +
                  "</a>";
                html += "</div>";
                ...
                if (response.length == 1) {
                  html +=
                    ...
                    elm["ADDRESS"] +
                    elm["FILE_NAME"] +
                    '" target="_blank" rel="noopener noreferrer"><img style="max-width:500px; max-height:500px;" src="' +
                    baseurl +
                    elm["ADDRESS"] +
                    elm["FILE_NAME"] +
                    '" alt="' +
                  ...
                  html +=
                    ...
                    elm["ADDRESS"] +
                    elm["FILE_NAME"] +
                    '" target="_blank" rel="noopener noreferrer"><img style="max-width:200px; max-height:200px;" src="' +
                    baseurl +
                    elm["ADDRESS"] +
                    elm["FILE_NAME"] +
                    '" alt="' +
                ...
                html +=
                  '<a href="' +
                  baseurl +
                  "OrderKebutuhanBarangDanJasa/Requisition/downloadAttachment?id-attachment=" +
                  elm["ATTACHMENT_ID"] +
                  '" target="_blank" title="Mohon maaf file ini tidak bisa dibuka karena bukan file gambar atau pdf. Klik untuk download" data-toggle="tooltip" filename="' +
                  elm["FILE_NAME"] +
  ...
  $(document).on("click", ".btnNormalOrderOKB", function () {
    window.location.href =
      baseurl + "OrderKebutuhanBarangDanJasa/Approver/PermintaanApproveNormal";
  });

  $(document).on("click", ".btnSusulanOrderOKB", function () {
    window.location.href =
      baseurl + "OrderKebutuhanBarangDanJasa/Approver/PermintaanApproveSusulan";
  });

  $(document).on("click", ".btnUrgentOrderOKB", function () {
    window.location.href =
      baseurl + "OrderKebutuhanBarangDanJasa/Approver/PermintaanApproveUrgent";
  });

  $(document).on("click", ".btnNormalOrderOKBPengelola", function () {
    window.location.href =
      baseurl + "OrderKebutuhanBarangDanJasa/Pengelola/OpenedOrderNormal";
  });

  $(document).on("click", ".btnSusulanOrderOKBPengelola", function () {
    window.location.href =
      baseurl + "OrderKebutuhanBarangDanJasa/Pengelola/OpenedOrderSusulan";
  });

  $(document).on("click", ".btnUrgentOrderOKBPengelola", function () {
    window.location.href =
      baseurl + "OrderKebutuhanBarangDanJasa/Pengelola/OpenedOrderUrgent";
  });
  // end
  ...
  function destinationLine(prn) {
    ...
    prn.find(".locationOKB").on("change", function () {
      ...
      $.ajax({
        type: "POST",
        url:
          baseurl + "OrderKebutuhanBarangDanJasa/Requisition/getSubinventory",
        data: {
          location: location,
    ...
    $(document).on("click", ".btnActUbahDescOKB", function () {
      ...
      if (desc2 != desc) {
        $.ajax({
          type: "POST",
          url:
            baseurl + "OrderKebutuhanBarangDanJasa/Approver/UbahDeskripsiOrder",
          data: {
            person_id: person_id,
            order_id: order_id,
            desc_baru: desc2,
...
function changeDescNewOrder(n) {
  //   console.log(n);
  var item_desc = $("#slcOKBNewOrderListNamaBarang" + n).val();

  $.ajax({
    type: "POST",
    dataType: "json",
    url: baseurl + "OrderKebutuhanBarangDanJasa/Requisition/getDescbiInvID",
    data: {
      item_desc: item_desc,
    },
  }).done((response) => {
    var panjang_item_desc = response.length;

    var sisa_panjang = 240 - panjang_item_desc;

    var desc = $("#txaOKBNewOrderDescription" + n).val();

    var panjang_desc = desc.length;

    if (panjang_desc > sisa_panjang) {
      //   alert("kepanjangan woy");
      Swal.fire({
        type: "error",
        title:
          "Deskripsi tidak boleh lebih dari " + sisa_panjang + " karakter!",
      }).then(() => {
        $("#txaOKBNewOrderDescription" + n).val("");
      });
    }
  });
}
