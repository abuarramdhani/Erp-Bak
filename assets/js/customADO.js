$(() => {
  ...
  $(document).on("ifChanged", ".chkADOPickedReleaseAll", function (e) {
    e.target.checked
      ? $(dataTableADODetailAllPageList)
          .find(".chkADOPickedRelease")
          .iCheck("check")
      : $(dataTableADODetailAllPageList)
          .find(".chkADOPickedRelease")
          .iCheck("uncheck");
  ...
  $(".btnADORequestApproveDO").on("click", function () {
    let data = {
      ...
      approver1Name: $(".slcADOAssignerList1")
        .find(":selected")
        .text()
        .split(" - ")
        .slice(-1)
        .pop(),
      approver2Name: $(".slcADOAssignerList2")
        .find(":selected")
        .text()
        .split(" - ")
        .slice(-1)
        .pop(),
      approver1Address: $(".slcADOAssignerList1")
        .find(":selected")
        .attr("address"),
      approver2Address: $(".slcADOAssignerList2")
        .find(":selected")
        .attr("address"),
    };
  ...
  $(".btnADORequestApproveSPB").on("click", function () {
    let data = {
      ...
      approverName: $(".slcADOAssignerList")
        .find(":selected")
        .text()
        .split(" - ")
        .slice(-1)
        .pop(),
      approverAddress: $(".slcADOAssignerList")
        .find(":selected")
        .attr("address"),
    };
    ...
    swalADOQuestionAjax(question, success, fail, url, data).then((resp) => {
      if (resp != "Cancelled" && resp != "Fail") {
        ...
        $(this)
          ...
          .html('<i class="fa fa-check-circle-o"></i>&nbsp; Approved');
    ...
    swalADOQuestionAjax(question, success, fail, url, data).then((resp) => {
      if (resp != "Cancelled" && resp != "Fail") {
        ...
        $(this)
          .removeClass("btn-primary")
          .addClass("btn-danger")
          .html('<i class="fa fa-remove"></i>&nbsp; Rejected');
    ...
    swalADOQuestionAjax(question, success, fail, url, data).then((resp) => {
      if (resp != "Cancelled" && resp != "Fail") {
        ...
        $(this)
          ...
          .html('<i class="fa fa-clock-o"></i>&nbsp; Pending');
  ...
  $(".btnADOLaunchRelease").on("click", function () {
    let data = {
      deliveryId: (() => {
        let checkedVal = [];
        $(dataTableADODetailAllPageList)
          .find(".chkADOPickedRelease:checked")
          .each(function () {
            checkedVal.push(
              $(this).parents("tr").find(".spnADODeliveryID").html()
            );
          });
        return checkedVal;
      })(),
  ...
  $(".btnADOCekStok").on("click", function () {
    ...
    if (gudang_pengirim) {
      ...
      $.ajax({
        ...
        data: { no_do, gudang_pengirim, org },
        dataType: "JSON",
  ...
  $(".btnADOAddNewRow").on("click", function () {
    ...
    dataTableADODetailList.row.add([
      rownum,
      '<select class="form-control-auto form-control nodoADO txtADODONumber no_do" id="DO_Numbrr' +
        rownum +
        '" onchange="cekOrgID(' +
        rownum +
        ')" style="width:200px;"></select>',
      '<input type="text" class="form-control-auto form-control txtADOItemName" id="txtADOItemName' +
        rownum +
        '">',
      '<input type="number" class="form-control-auto form-control txtADOQty" id="txtADOQty' +
        rownum +
        '">',
      '<input type="text" class="form-control-auto form-control txtADOUOM" id="txtADOUOM' +
        rownum +
        '">',
      '<input type="text" class="form-control-auto form-control txtADOShopName" id="txtADOShopName' +
        rownum +
        '">',
      '<input type="text" class="form-control-auto form-control txtADOCity" id="txtADOCity' +
        rownum +
        '">',
  ...
  $(".btnADOSaveUpdate").on("click", function () {
    let data = {
      ...
      newLine: (() => {
        let data = [];
        let line = [];
        $(dataTableADODetailList.rows('[data-type="update"]').nodes()).each(
          function () {
      ...
      deleteLine: (() => {
        let data = [];
        $(dataTableADODetailList.rows('[data-type="delete"]').nodes()).each(
          function () {
            data.push({
              lineNumber: $(this).attr("data-line"),
            });
...
function cekOrgID(i) {
  var value = $("#DO_Numbrr" + i).val();
  $.ajax({
    type: "POST",
    url: baseurl + "ApprovalDO/DPBKHS/cekOrgID",
    data: { value: value },
    dataType: "json",
  }).done((resp) => {
    if (i == 1) {
      ...
      if (resp != org) {
        Swal.fire({
          position: "top",
          type: "error",
          title: "Organization harus Sama !",
          showConfirmButton: true,
        }).then(() => {
          //   window.location.reload();
          $("#DO_Numbrr" + i).select2("val", "");
          //   $("#txtADOItemName" + i).val("");
          //   $("#txtADOQty" + i).val("");
          //   $("#txtADOUOM" + i).val("");
          //   $("#txtADOShopName" + i).val("");
          //   $("#txtADOCity" + i).val("");
        });
      }
    }
