...
function sortMapDescendingByValue(a, b) {
  return b[1] - a[1];
}
...
function minMaxId(a) {
  ...
  for (let i = 0; i < distributionTime.length; i++)
    if (distributionTime[i] != null) input[i].value = distributionTime[i];
...
function copyAutoWaktuDistribusi(a) {
  ...
  for (let i = 0; i < xMin.length; i++)
    if (xMin[i] != null) set[i].value = Number(xMin[i]) + Number(autoDistribusi[i]);
      ...
      processResults: function(seksi) {
        return {
          results: $.map(seksi, function(obj) {
            return {
              id: obj.SEKSI_PENGEBON,
              text: obj.SEKSI_PENGEBON
            };
          })
        };
      }
...
function attachRowObservation_new() {
  ...
  if (indx < maxRow) {
    newRow.insertBefore($('#tblObservasi tbody tr:nth(' + indx + ')'));
